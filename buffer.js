...
var Buffer = Buffer || (function()
{
	...
	function alloc(that, size, fill, encoding)
	{
		...
		if (typeof fill !== 'undefined') {
			...
			return typeof encoding === 'string'
				? createBuffer(that, size).fill(fill, encoding) : createBuffer(that, size).fill(fill);
		}
		return createBuffer(that, size);
	}
	...
	function fromArrayBuffer(that, array, byteOffset, length)
	{
		...
		if (typeof byteOffset === 'undefined' && typeof length === 'undefined') {
	...
	function checked(length)
	{
		...
		if (length >= Buffer.kMaxLength) {
			throw new RangeError('Attempt to allocate Buffer larger than maximum ' +
								'size: 0x' + Buffer.kMaxLength.toString(16) + ' bytes');
		}
		return length | 0;
	}
	...
	Buffer.prototype.write = function(string, offset, length, encoding)
	{
		...
		if (typeof length === 'undefined' || length > remaining) length = remaining;
	...
	var base64 = (function() {
		...
		function getLens(b64)
		{
			...
			var byteLen = ((validLen + placeHoldersLen) * 3 / 4) - placeHoldersLen;

			return [validLen, placeHoldersLen, byteLen];
		}
...
//			var arr = new Arr(_byteLength(b64, validLen, placeHoldersLen));
	...
	got from https://github.com/feross/ieee754
	*/
