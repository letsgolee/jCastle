/**
 * jCastle Mcrypt Module - AES (Fast version / supports 16 block size only)
 * 
 * @author Jacob Lee
 *
...
/* the fast version supports only 16 block size */
/* be careful with this version, for it needs second parameter for key expanding. */

var jCastle = require('../jCastle');
require('../util');

jCastle.algorithm.aes = class 
{
	/**
	...
	//	}
		...
		this.expandKey(enckey, isEncryption);
		return this;
	}
	...
	encryptBlock(input)
	{
		var B = jCastle.util.byte;

		var C0 = input.readInt32LE(0);
		var C1 = input.readInt32LE(4);
		var C2 = input.readInt32LE(8);
		var C3 = input.readInt32LE(12);
		...
		var t0, t1, t2, t3;
		var r = 0;
		var T = jCastle.algorithm.aes.precomputed_table;
		var S = jCastle.algorithm.aes.sbox;

		while (r < this.rounds - 1)
		{
			t0 = C0 ^ this.roundKey[r][0];
			t1 = C1 ^ this.roundKey[r][1];
			t2 = C2 ^ this.roundKey[r][2];
			t3 = C3 ^ this.roundKey[r++][3];

			C0 = T[0][B(t0, 0)] ^ T[1][B(t1, 1)] ^ T[2][B(t2, 2)] ^ T[3][B(t3, 3)];
			C1 = T[0][B(t1, 0)] ^ T[1][B(t2, 1)] ^ T[2][B(t3, 2)] ^ T[3][B(t0, 3)];
			C2 = T[0][B(t2, 0)] ^ T[1][B(t3, 1)] ^ T[2][B(t0, 2)] ^ T[3][B(t1, 3)];
			C3 = T[0][B(t3, 0)] ^ T[1][B(t0, 1)] ^ T[2][B(t1, 2)] ^ T[3][B(t2, 3)];
		}
		...
		C0 = S[B(t0, 0)] ^ (S[B(t1, 1)] << 8) ^ (S[B(t2, 2)] << 16) ^ (S[B(t3, 3)] << 24) ^ this.roundKey[r][0];
		C1 = S[B(t1, 0)] ^ (S[B(t2, 1)] << 8) ^ (S[B(t3, 2)] << 16) ^ (S[B(t0, 3)] << 24) ^ this.roundKey[r][1];
		C2 = S[B(t2, 0)] ^ (S[B(t3, 1)] << 8) ^ (S[B(t0, 2)] << 16) ^ (S[B(t1, 3)] << 24) ^ this.roundKey[r][2];
		C3 = S[B(t3, 0)] ^ (S[B(t0, 1)] << 8) ^ (S[B(t1, 2)] << 16) ^ (S[B(t2, 3)] << 24) ^ this.roundKey[r][3];

		var output = Buffer.alloc(16);

		output.writeInt32LE(C0, 0);
		output.writeInt32LE(C1, 4);
		output.writeInt32LE(C2, 8);
		output.writeInt32LE(C3, 12);
	...
	decryptBlock(input)
	{
		...
		var t0, t1, t2, t3;
		var r = this.rounds;
		var Ti = jCastle.algorithm.aes.precomputed_table_inverse;
		var Si = jCastle.algorithm.aes.sbox_inverse;
		...
		while (r > 1)
		{
			...
			C0 = Ti[0][B(t0, 0)] ^ Ti[1][B(t3, 1)] ^ Ti[2][B(t2, 2)] ^ Ti[3][B(t1, 3)];
			C1 = Ti[0][B(t1, 0)] ^ Ti[1][B(t0, 1)] ^ Ti[2][B(t3, 2)] ^ Ti[3][B(t2, 3)];
			C2 = Ti[0][B(t2, 0)] ^ Ti[1][B(t1, 1)] ^ Ti[2][B(t0, 2)] ^ Ti[3][B(t3, 3)];
			C3 = Ti[0][B(t3, 0)] ^ Ti[1][B(t2, 1)] ^ Ti[2][B(t1, 2)] ^ Ti[3][B(t0, 3)];
		}
		...
		C0 = Si[B(t0, 0)] ^ (Si[B(t3, 1)] << 8) ^ (Si[B(t2, 2)] << 16) ^ (Si[B(t1, 3)] << 24) ^ this.roundKey[r][0];
		C1 = Si[B(t1, 0)] ^ (Si[B(t0, 1)] << 8) ^ (Si[B(t3, 2)] << 16) ^ (Si[B(t2, 3)] << 24) ^ this.roundKey[r][1];
		C2 = Si[B(t2, 0)] ^ (Si[B(t1, 1)] << 8) ^ (Si[B(t0, 2)] << 16) ^ (Si[B(t3, 3)] << 24) ^ this.roundKey[r][2];
		C3 = Si[B(t3, 0)] ^ (Si[B(t2, 1)] << 8) ^ (Si[B(t1, 2)] << 16) ^ (Si[B(t0, 3)] << 24) ^ this.roundKey[r][3];
	...
	{
		var	key_len = key.length / 4;  // key length in words
		
	//	if (((key_len != 4) && (key_len != 6) && (key_len != 8)) || key.length % 4) {
	//		throw new Error("Key length not 128/192/256 bits.");
	//	}

		this.rounds = key_len + 6;  // This is not always true for the generalized AES that allows larger block sizes

		var round_key = new Array(this.rounds + 1);
		for (var i = 0; i < round_key.length; i++) {
			round_key[i] = new Array(4);
		}
		...
		var i = 0;
		while (i < key.length) {
			//round_key[t >>> 2][t & 3] = jCastle.util.load32(key, i);
			round_key[t >>> 2][t & 3] = key.readInt32LE(i);
			i += 4;
			t++;
		}
		...
		for (var i = key_len; i < k; i++) {
			var temp = round_key[(i - 1) >>> 2][(i - 1) & 3];
			if ((i % key_len) == 0) {
				temp = this.subWord(jCastle.util.rotr32(temp, 8)) ^ jCastle.algorithm.aes.rcon[(i / key_len) - 1];
			} else if ((key_len > 6) && ((i % key_len) == 4)) {
				temp = this.subWord(temp);
			}

			round_key[i >>> 2][i & 3] = round_key[(i - key_len) >>> 2][(i - key_len) & 3] ^ temp;
		}

		if (!isEncryption) {
			for (var j = 1; j < this.rounds; j++) {
				for (i = 0; i < 4; i++) {
					round_key[j][i] = this.inv_mcol(round_key[j][i]);
				}
			}
		}

		this.roundKey = round_key;
	}

/*
	FFmulX(x)
	{
		return (((x & jCastle.algorithm.aes.m2) << 1) ^ (((x & jCastle.algorithm.aes.m1) >>> 7) * jCastle.algorithm.aes.m3));
	}

	FFmulX2(x)
	{
		var t0  = (x & jCastle.algorithm.aes.m5) << 2;
		var t1  = (x & jCastle.algorithm.aes.m4);
	...
	{
		var t0  = x;
		var t1  = t0 ^ jCastle.util.rotr32(t0, 8);

		t0 ^= this.FFmulX(t1);
		t1 ^= this.FFmulX2(t0);
		t0 ^= t1 ^ jCastle.util.rotr32(t1, 16);
...
*/
	...
	inv_mcol(x)
	{
		var U = jCastle.algorithm.aes.precomputed_mult;

		return U[0][x & 0xff] ^ U[1][(x >>> 8) & 0xff] ^ U[2][(x >>> 16) & 0xff] ^ U[3][(x >>> 24) & 0xff];
	}

	subWord(x)
	{
		var S = jCastle.algorithm.aes.sbox;
		var i0 = x, i1 = x >>> 8, i2 = x >>> 16, i3 = x >>> 24;

		i0 = S[i0 & 0xff] & 0xff; i1 = S[i1 & 0xff] & 0xff; i2 = S[i2 & 0xff] & 0xff; i3 = S[i3 & 0xff] & 0xff;
...
Object.assign(jCastle.algorithm.aes, 
{
	sbox: [
...
// precomputation tables of calculations for rounds
	...
	/*
	jCastle.algorithm.aes.m1 = 0x80808080;
	jCastle.algorithm.aes.m2 = 0x7f7f7f7f;
	jCastle.algorithm.aes.m3 = 0x0000001b;
	jCastle.algorithm.aes.m4 = 0xC0C0C0C0;
	jCastle.algorithm.aes.m5 = 0x3f3f3f3f;
	*/
	precomputed_mult: [
		// U0
		[
...
jCastle._algorithmInfo['aes-128'] = {
	algorithm_type: 'crypt',
	block_size: 16,
	key_size: 16,
	min_key_size: 16,
...
jCastle._algorithmInfo['aes-192'] = {
	algorithm_type: 'crypt',
	block_size: 16,
	key_size: 24,
	min_key_size: 24,
...
jCastle._algorithmInfo['aes-256'] = {
	algorithm_type: 'crypt',
	block_size: 16,
	key_size: 32,
	min_key_size: 32,
...
module.exports = jCastle.algorithm.aes;