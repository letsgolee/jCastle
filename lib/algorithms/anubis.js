/**
 * Javascript jCastle Mcrypt Module - Anubis 
 * 
 * @author Jacob Lee
 *
...
jCastle.algorithm.anubis = class
{
	/**
    ...
    constructor(algo_name, options = {})
    {
        ...
        if ('tweaked' in options) {
            this.tweaked = options.tweaked;
        }
    }
	...
	*/
	...
	{
		var N, R, i, pos;
		var kappa = new Array(10);
		var inter = new Array(10);
		var v, K0, K1, K2, K3;
		var keylen = key.length;
		
		var t4 = jCastle.algorithm.anubis.t4;
		var t5 = jCastle.algorithm.anubis.t5;
		...
		for (var r = 0; r <= R; r++) {
			...
			*/
			...
			this.encRoundKeys[r][0] = K0;
			this.encRoundKeys[r][1] = K1;
			this.encRoundKeys[r][2] = K2;
			this.encRoundKeys[r][3] = K3;
			...
			for (i = 0; i < N; i++) {
				var j = i;
				inter[i]  = jCastle.algorithm.anubis.t0[(kappa[j--] >>> 24) & 0xff]; if (j < 0) j = N - 1;
				inter[i] ^= jCastle.algorithm.anubis.t1[(kappa[j--] >>> 16) & 0xff]; if (j < 0) j = N - 1;
				inter[i] ^= jCastle.algorithm.anubis.t2[(kappa[j--] >>>  8) & 0xff]; if (j < 0) j = N - 1;
				inter[i] ^= jCastle.algorithm.anubis.t3[(kappa[j  ]	  ) & 0xff];
			}
			kappa[0] = inter[0] ^ jCastle.algorithm.anubis.rc[r];
		...
		this.decRoundKeys = new Array(R);

		// generate inverse key schedule: K'^0 = K^R, K'^R = K^0, K'^r = theta(K^{R-r}):
		this.decRoundKeys[0] = [];
		this.decRoundKeys[R] = [];
		for (i = 0; i < 4; i++) {
			this.decRoundKeys[0][i] = this.encRoundKeys[R][i];
			this.decRoundKeys[R][i] = this.encRoundKeys[0][i];
		}
		for (r = 1; r < R; r++) {
			this.decRoundKeys[r] = [];
			for (i = 0; i < 4; i++) {
				v = this.encRoundKeys[R - r][i];
				this.decRoundKeys[r][i] =
					jCastle.algorithm.anubis.t0[t4[(v >>> 24) & 0xff] & 0xff] ^
					jCastle.algorithm.anubis.t1[t4[(v >>> 16) & 0xff] & 0xff] ^
					jCastle.algorithm.anubis.t2[t4[(v >>>  8) & 0xff] & 0xff] ^
					jCastle.algorithm.anubis.t3[t4[(v	  ) & 0xff] & 0xff];
			}
		}
	}
	...
	cryptBlock(direction, input)
	{
		...
		var inter = new Array(4);
		var R = this.rounds;
		var roundKey = direction ? this.encRoundKeys : this.decRoundKeys;
		...
		for (i = 0, pos = 0; i < 4; i++, pos += 4) {
			state[i] = input.readInt32BE(pos) ^ roundKey[0][i];
		}
		...
		for (r = 1; r < R; r++) {
			inter[0] =
				jCastle.algorithm.anubis.t0[(state[0] >>> 24) & 0xff] ^
				jCastle.algorithm.anubis.t1[(state[1] >>> 24) & 0xff] ^
				jCastle.algorithm.anubis.t2[(state[2] >>> 24) & 0xff] ^
				jCastle.algorithm.anubis.t3[(state[3] >>> 24) & 0xff] ^
				roundKey[r][0];
			inter[1] =
				jCastle.algorithm.anubis.t0[(state[0] >>> 16) & 0xff] ^
				jCastle.algorithm.anubis.t1[(state[1] >>> 16) & 0xff] ^
				jCastle.algorithm.anubis.t2[(state[2] >>> 16) & 0xff] ^
				jCastle.algorithm.anubis.t3[(state[3] >>> 16) & 0xff] ^
				roundKey[r][1];
			inter[2] =
				jCastle.algorithm.anubis.t0[(state[0] >>>  8) & 0xff] ^
				jCastle.algorithm.anubis.t1[(state[1] >>>  8) & 0xff] ^
				jCastle.algorithm.anubis.t2[(state[2] >>>  8) & 0xff] ^
				jCastle.algorithm.anubis.t3[(state[3] >>>  8) & 0xff] ^
				roundKey[r][2];
			inter[3] =
				jCastle.algorithm.anubis.t0[(state[0]	  ) & 0xff] ^
				jCastle.algorithm.anubis.t1[(state[1]	  ) & 0xff] ^
				jCastle.algorithm.anubis.t2[(state[2]	  ) & 0xff] ^
				jCastle.algorithm.anubis.t3[(state[3]	  ) & 0xff] ^
				roundKey[r][3];
			state[0] = inter[0];
			state[1] = inter[1];
			state[2] = inter[2];
		...
		// last round:
		inter[0] =
			(jCastle.algorithm.anubis.t0[(state[0] >>> 24) & 0xff] & 0xff000000) ^
			(jCastle.algorithm.anubis.t1[(state[1] >>> 24) & 0xff] & 0x00ff0000) ^
			(jCastle.algorithm.anubis.t2[(state[2] >>> 24) & 0xff] & 0x0000ff00) ^
			(jCastle.algorithm.anubis.t3[(state[3] >>> 24) & 0xff] & 0x000000ff) ^
			roundKey[R][0];
		inter[1] =
			(jCastle.algorithm.anubis.t0[(state[0] >>> 16) & 0xff] & 0xff000000) ^
			(jCastle.algorithm.anubis.t1[(state[1] >>> 16) & 0xff] & 0x00ff0000) ^
			(jCastle.algorithm.anubis.t2[(state[2] >>> 16) & 0xff] & 0x0000ff00) ^
			(jCastle.algorithm.anubis.t3[(state[3] >>> 16) & 0xff] & 0x000000ff) ^
			roundKey[R][1];
		inter[2] =
			(jCastle.algorithm.anubis.t0[(state[0] >>>  8) & 0xff] & 0xff000000) ^
			(jCastle.algorithm.anubis.t1[(state[1] >>>  8) & 0xff] & 0x00ff0000) ^
			(jCastle.algorithm.anubis.t2[(state[2] >>>  8) & 0xff] & 0x0000ff00) ^
			(jCastle.algorithm.anubis.t3[(state[3] >>>  8) & 0xff] & 0x000000ff) ^
			roundKey[R][2];
		inter[3] =
			(jCastle.algorithm.anubis.t0[(state[0]	  ) & 0xff] & 0xff000000) ^
			(jCastle.algorithm.anubis.t1[(state[1]	  ) & 0xff] & 0x00ff0000) ^
			(jCastle.algorithm.anubis.t2[(state[2]	  ) & 0xff] & 0x0000ff00) ^
			(jCastle.algorithm.anubis.t3[(state[3]	  ) & 0xff] & 0x000000ff) ^
			roundKey[R][3];

		for (i = 0, pos = 0; i < 4; i++, pos += 4) {
			output.writeInt32BE(inter[i], pos, true);
		}
...
jCastle._algorithmInfo['anubis-128'] = {
	algorithm_type: 'crypt',
	block_size: 16,
	key_size: 16,
	min_key_size: 16,
...
jCastle._algorithmInfo['anubis-160'] = {
	algorithm_type: 'crypt',
	block_size: 16,
	key_size: 20,
	min_key_size: 20,
...
jCastle._algorithmInfo['anubis-192'] = {
	algorithm_type: 'crypt',
	block_size: 16,
	key_size: 24,
	min_key_size: 24,
...
jCastle._algorithmInfo['anubis-224'] = {
	algorithm_type: 'crypt',
	block_size: 16,
	key_size: 28,
	min_key_size: 28,
...
jCastle._algorithmInfo['anubis-256'] = {
	algorithm_type: 'crypt',
	block_size: 16,
	key_size: 32,
	min_key_size: 32,
...
jCastle._algorithmInfo['anubis-288'] = {
	algorithm_type: 'crypt',
	block_size: 16,
	key_size: 36,
	min_key_size: 36,
...
jCastle._algorithmInfo['anubis-320'] = {
	algorithm_type: 'crypt',
	block_size: 16,
	key_size: 40,
	min_key_size: 40,
...
module.exports = jCastle.algorithm.anubis;