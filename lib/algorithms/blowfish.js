/**
 * Javascript jCastle Module - Blowfish / Blowfish-Compat
 * 
 * @author Jacob Lee
 *
...
jCastle.algorithm.blowfish = class
{
	/**
	...
	encryptBlock(input)
	{
		...
		var size = jCastle.algorithm.blowfish.pboxSize;
		var Xl = input.readInt32BE(0);
		var Xr = input.readInt32BE(4);

		if (this.algoName == 'blowfish-compat') {
			Xl = jCastle.util.bswap32(Xl);
			Xr = jCastle.util.bswap32(Xr);
		}
		...
		output.writeInt32BE(Xl, 0);
		output.writeInt32BE(Xr, 4);
	...
	decryptBlock(input)
	{
		...
		var output = Buffer.alloc(input.length);
		var size = jCastle.algorithm.blowfish.pboxSize;
		var Xl = jCastle.util.load32b(input, 0);
		var Xr = jCastle.util.load32b(input, 4);
 ...
 */
	...
	F(x)
	{
		return  ((this.sbox[0][(x >>> 24) & 0xff] + this.sbox[1][(x >>> 16) & 0xff]) ^ this.sbox[2][(x >>>  8) & 0xff]) + this.sbox[3][x & 0xff];
	}
	...
	expandKey(key, isEncryption)
	{
		...
		this.pbox = new Array(18);

		for (var i = 0; i < 4; i++) {
			this.sbox[i] = new Array(256);
		}

	/* Initialize s-boxes without file read. */
		for (i = 0; i < 256; i++) {
			this.sbox[0][i] = jCastle.algorithm.blowfish.ks0[i];
			this.sbox[1][i] = jCastle.algorithm.blowfish.ks1[i];
			this.sbox[2][i] = jCastle.algorithm.blowfish.ks2[i];
			this.sbox[3][i] = jCastle.algorithm.blowfish.ks3[i];
		}

	/* P-boxes */
		for (i = 0; i < 18; i++) {
			this.pbox[i] = jCastle.algorithm.blowfish.pi[i];
		}

		j = 0;
		for (i = 0; i < size + 2; ++i) {
			data = 0x00000000;
			data = (data << 8) | key[(j) % len];
			data = (data << 8) | key[(j + 1) % len];
			data = (data << 8) | key[(j + 2) % len];
			data = (data << 8) | key[(j + 3) % len];
		...
		for (i = 0; i < size + 2; i += 2) {
			this.enblf_noswap(datarl);
			this.pbox[i] = datarl[0];
			this.pbox[i + 1] = datarl[1];
		}
		...
		for (i = 0; i < 4; ++i) {
			for (j = 0; j < 256; j += 2) {
				this.enblf_noswap(datarl);
				this.sbox[i][j] = datarl[0];
				this.sbox[i][j + 1] = datarl[1];
			}
		}
	}
}
...
jCastle.algorithm.blowfish.pboxSize = 16;

jCastle.algorithm.blowfish.ks0 = [
...
jCastle.algorithm.blowfish.ks1 = [
...
jCastle.algorithm.blowfish.ks2 = [
...
jCastle.algorithm.blowfish.ks3 = [
...
jCastle._algorithmInfo['blowfish'] = {
	...
	max_key_size: 56,
	padding: 'zeros',
	object_name: 'blowfish'
};

jCastle._algorithmInfo['blowfish-compat'] = {
	algorithm_type: 'crypt',
	block_size: 8,
	key_size: 56,
	min_key_size: 16,
...
module.exports = jCastle.algorithm.blowfish;