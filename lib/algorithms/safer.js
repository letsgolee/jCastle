 ...
 */
	...
	expandKey(key, isEncryption)
	{
		...
		switch (bits) {
			case 128:
				var nof_rounds = strengthened ? jCastle.algorithm.safer.SK128_DEFAUT_ROUNDS : jCastle.algorithm.safer.K128_DEFAUT_ROUNDS;
				for(var i = 0; i < 8; i++) {
					userkey_1[i] = key[i];
					userkey_2[i] = key[i+8];
				}
				break;
			case 64:
				var nof_rounds = strengthened ? jCastle.algorithm.safer.SK64_DEFAUT_ROUNDS : jCastle.algorithm.safer.K64_DEFAUT_ROUNDS;
				for(var i = 0; i < 8; i++) {
					userkey_1[i] = key[i];
					userkey_2[i] = key[i];
				}
