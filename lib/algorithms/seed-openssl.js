...
/* 
OpenSSL's Seed algorithm seems to give different results from KISA's Seed.
This Seed algorithm comes from OpenSSL.
https://github.com/openssl/openssl/tree/master/crypto/seed
*/
...
//		if (!this.rounds) this.rounds = jCastle._algorithmInfo[this.algoName].rounds;
	...
	encryptBlock(input)
	{
		...
		x[0] = input.readInt32BE(0);
		x[1] = input.readInt32BE(4);
		x[2] = input.readInt32BE(8);
		x[3] = input.readInt32BE(12);

		for (var i = 0; i < 30; i += 4) {
			this.E_SEED(x, 0, 1, 2, 3, i);
			this.E_SEED(x, 2, 3, 0, 1, i + 2);
		}

		output.writeInt32BE(x[2], 0);
		output.writeInt32BE(x[3], 4);
		output.writeInt32BE(x[0], 8);
		output.writeInt32BE(x[1], 12);
	...
	expandKey(key)
	{
		...
		x[0] = key.readInt32BE(0);
		x[1] = key.readInt32BE(4);
		x[2] = key.readInt32BE(8);
		x[3] = key.readInt32BE(12);

		t[0] = (x[0] + x[2] - KC0) & 0xffffffff;
		t[1] = (x[1] - x[3] + KC0) & 0xffffffff;
		this.KEYUPDATE_TEMP(t, 0);
		this.KEYSCHEDULE_UPDATE1(t, x, 0, 1, 2, 3, KC1);
		this.KEYUPDATE_TEMP(t, 2);

		for (var i = 2; i < 16; i += 2) {
			this.KEYSCHEDULE_UPDATE0(t, x, 0, 1, 2, 3, KC[i]);
			this.KEYUPDATE_TEMP(t, i * 2);
			this.KEYSCHEDULE_UPDATE1(t, x, 0, 1, 2, 3, KC[i + 1]);
			this.KEYUPDATE_TEMP(t, i * 2 + 2);
		}
	}
	...
	G_FUNC(v)
	{
		var SS = jCastle.algorithm.seed.SBOX;
		return SS[0][v & 0xff] ^ SS[1][(v>>>8) & 0xff] ^ SS[2][(v>>>16) & 0xff] ^ SS[3][(v>>>24) & 0xff];
	}

	KEYSCHEDULE_UPDATE0(T, X, p0, p1, p2, p3, KC)
	{
		T[0] = X[p2];
		X[p2] = ((X[p2]<<8) ^ (X[p3]>>>24)) & 0xffffffff;
		X[p3] = ((X[p3]<<8) ^ (T[0]>>>24))  & 0xffffffff;
		T[0] = (X[p0] + X[p2] - KC)         & 0xffffffff;
		T[1] = (X[p1] + KC - X[p3])         & 0xffffffff;
	}

	KEYSCHEDULE_UPDATE1(T, X, p0, p1, p2, p3, KC)
	{
		T[0] = X[p0];
		X[p0] = ((X[p0]>>>8) ^ (X[p1]<<24)) & 0xffffffff;
		X[p1] = ((X[p1]>>>8) ^ (T[0]<<24))  & 0xffffffff;
		T[0] = (X[p0] + X[p2] - KC)         & 0xffffffff;
		T[1] = (X[p1] + KC - X[p3])         & 0xffffffff;
	}
	...
	KEYUPDATE_TEMP(t, i)
	{
		this.roundKey[i] = this.G_FUNC(t[0]);
		this.roundKey[i + 1] = this.G_FUNC(t[1]);
	}

	E_SEED(X, p0, p1, p2, p3, i)
	{
		var t0 = X[p2] ^ this.roundKey[i];
		var t1 = X[p3] ^ this.roundKey[i + 1];
		t1 ^= t0;
		t1 = this.G_FUNC(t1);
		t0 = (t0 + t1) & 0xffffffff;
		t0 = this.G_FUNC(t0);
...
Object.assign(jCastle.algorithm.seed,
{
	SBOX: [
