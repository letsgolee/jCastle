...
*/
	...
	parse(cnf_content, env_variables)
	{
		...
		if (!jCastle.util.isString(cnf_content)) {
		...
		if (typeof env_variables == 'object') {
			for (var item in env_variables) {
				...
				cnf_content = cnf_content.replace(new RegExp(it, 'g'), env_variables[item]);
			}
		}

		this._buildConfig(cnf_content);
	...
	_buildConfig(content)
	{
		...
		this._searchEnvVariables();

		// replace strings registerd in env_variables...
		this._replaceEnvVariables();
	...
	{
		// if (!section) section = this.contents;
		...
		for (var s in section) {
			...
			if (jCastle.util.isString(section[s])) {
				...
				var m = /\$([a-z\_][a-z\_0-9]+)/i.exec(section[s]);
				// console.log('section[s]: ', section[s], ', ', 'm: ', m);
				if (!m) continue;
	...
	_replaceEnvVariables(section = null)
	{
		if (!section) section = this.contents;
								...
								if ('userNotice' in info) {
									for (var j = 0; j < info.userNotice.length; j++) {
										...
										if ('noticeNumbers' in info.userNotice[j]) {
											...
											// do not use Object.assign, for it gives an unexpected result.
...
*/
	_parseASN1Items(obj)
	{
		...
		for(var i in obj) {
			...
			var value = obj[i].split(':');
			var type = value.shift().trim();
			...
			switch (type.toUpperCase()) {
				...
				default:
					var m = /^DER\:(.*)/.exec(value);
					var type = this._getASN1Type(type);
