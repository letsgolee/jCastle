...
*/
		switch (type) {
			case jCastle.certificate.typeCRT:
				var tbs_info = this._parseTbsCertificate(sequence.items[0]);
				break;
			case jCastle.certificate.typeCSR:
				var tbs_info = this._parseCsrTbsCertificate(sequence.items[0]);
				break;		
			case jCastle.certificate.typeCRL:
				var tbs_info = this._parseCrlTbsCertificate(sequence.items[0]);
				break;
			default: 
...
*/
	...
	request(req_info, options = {})
	{
		...
		if ('signKey' in options) this.setSignKey(options.signKey);
	...
	issue(csr_pem, issue_info, options = {})
	{
		...
		if ('issuerCertificate' in options) {
			if (jCastle.util.isString(options.issuerCertificate)) {
				var cert_info = new jCastle.certificate().parse(options.issuerCertificate);
			} else {
				var cert_info = options.issuerCertificate;
			}
			...
			switch (cert_build.algo.signAlgo) {
				case 'RSASSA-PKCS1-V1_5':
					if ('padding' in cert_build.tbs.subjectPublicKeyInfo && 
						...
						if (!('extensions' in cert_build.tbs)) cert_build.tbs.extensions = {};
						var ext = cert_build.tbs.extensions;
...
*/
	...
	revoke(revoked_list, revoke_info, options = {})
	{
		...
		if ('certificate' in options) {
			if (jCastle.util.isString(options.certificate)) {
				var cert_info = new jCastle.certificate().parse(options.certificate);
			} else {
				var cert_info = options.certificate;
			}
	...
	verify(pem, pub_pki = null, format = 'auto')
	{
		if (!pem && this.certInfo) {
			var cert_info = this.certInfo;
		} else {
			var cert_info = new jCastle.certificate().parse(pem, format);
		}
	...
	isValid(pem, crl_pem)
	{
		...
		serial = cert_info.tbs.serialNumber instanceof BigInteger ? cert_info.tbs.serialNumber : (
			typeof cert_info.tbs.serialNumber == 'number' ? new BigInteger(cert_info.tbs.serialNumber) : (
				/^[0-9A-F]+$/i.test(cert_info.tbs.serialNumber) ? 
					new BigInteger(cert_info.tbs.serialNumber, 16) : BigInteger.fromByteArray(Buffer.from(cert_info.tbs.serialNumber, 'latin1'))
			)
		);

		for (var i = 0; i < crl_info.tbs.revokedCertificates.length; i++) {
			var revoked = crl_info.tbs.revokedCertificates[i];
			var revoked_serial = revoked.userCertificate instanceof BigInteger ? revoked.userCertificate : (
				typeof revoked.userCertificate == 'number' ? new BigInteger(revoked.userCertificate) : (
					/^[0-9A-F]+$/i.test(revoked.userCertificate) ?
						new BigInteger(revoked.userCertificate, 16) : BigInteger.fromByteArray(Buffer.from(revoked.userCertificate, 'latin1'))
				)
			);
...
*/
		...
		if (obj.type == jCastle.asn1.tagInteger) {
			...
			//tbs_info.serialNumber = new BigInteger(jCastle.encoding.hex.encode(obj.value), 16);
...
*/
		...
		if (cert_info.type == jCastle.certificate.typeCRT) {
			...
			if (!cert_info.tbs.serialNumber) { // generate serialNumber
				...
				var serial = BigInteger.fromByteArrayUnsigned(bytes);
				//console.log(serial.gt(BigInteger.ZERO));
...
*/
		...
		if (cert_info.type == jCastle.certificate.typeCRT) {
			tbsCertificate.items.push({
				type: jCastle.asn1.tagInteger,
				value: cert_info.tbs.serialNumber
			});
		}
...
*/
	reviseRevokedCertificates: function(revoked_list)
	{
		...
		for (var i = 0; i < revoked_list.length; i++) {
			...
			if (jCastle.util.isString(l.userCertificate) && /^[0-9A-F]+$/i.test(l.userCertificate)) {
				l.userCertificate = new BigInteger(l.userCertificate, 16);
			}
	...
	transformConfigExtensions: function(cert_build, config, req_pki, sign_pki, cert_info)
	{
		switch (cert_build.type) {
			case jCastle.certificate.typeCSR:
				var extensions = cert_build.tbs.extensionRequest; break;
			case jCastle.certificate.typeCRL:
				var extensions = cert_build.tbs.crlExtensions; break;
			default:
				var extensions = cert_build.tbs.extensions; break;
		}
			...
			switch (ext) {
				...
				case 'authorityKeyIdentifier':
					...
					if (cert_info && 'authorityCertSerialNumber' in cert_info && cert_info.authorityCertSerialNumber) {
						var serial = jCastle.util.toBigInteger(cert_info.authorityCertSerialNumber);
...
*/
		if (jCastle.oid.getOID('rsaPSS') == sequence.items[0].value) {
			if (sequence.items[1].items[0].items[0].items[0].type == jCastle.asn1.tagOID) {
				var hash_name = jCastle.digest.getHashNameByOID(sequence.items[1].items[0].items[0].items[0].value);
...
*/
							...
							switch (code) {
								case 0: var reasonCode = 'unspecified'; break;
								case 1: var reasonCode = 'keyCompromise'; break;
								case 2: var reasonCode = 'cACompromise'; break;
								case 3: var reasonCode = 'affiliationChanged'; break;
								case 4: var reasonCode = 'superseded'; break;
								case 5: var reasonCode = 'cessationOfOperation'; break;
								case 6: var reasonCode = 'certificateHold'; break;
								case 8: var reasonCode = 'removeFromCRL'; break;
								case 9: var reasonCode = 'privilegeWithdrawn'; break;
								case 10: var reasonCode = 'aACompromise'; break;
							}
					...
					switch (j) {
						case 'cRLReason':
							switch (revoked.crlEntryExtensions.cRLReason) {
								case 'unspecified':				var code = 0; break;
								case 'keyCompromise':			var code = 1; break;
								case 'cACompromise':			var code = 2; break;
								case 'affiliationChanged':		var code = 3; break;
								case 'superseded':				var code = 4; break;
								case 'cessationOfOperation':	var code = 5; break;
								case 'certificateHold':			var code = 6; break;
								case 'removeFromCRL':			var code = 8; break;
								case 'privilegeWithdrawn':		var code = 9; break;
								case 'aACompromise':			var code = 10; break;
							}
...
*/
			...
			case 7:
				...
				if (obj.value.length == 4 || obj.value.length == 8) {
					var ip_address = jCastle.util.loadIPv4(obj.value);
				} else { // 16 bytes

					var ip_address = jCastle.util.loadIPv6(obj.value);
				}
...
*/
	...
	schema: function(obj)
	{
		...
		if (obj.name in jCastle.certificate.otherNameRules) {
			var schema = jCastle.certificate.otherNameRules[obj.name].schema(obj);
...
*/
				...
				if (seq.items[idx] && seq.items[idx].type == 1) {
					var mgf = jCastle.oid.getName(seq.items[1].items[0].items[0].value);
			...
			case 'KCDSA':
				var encryptionInfoSchema = {
					...
					items: [{
						...
						items: [{
							type: jCastle.asn1.tagInteger,
							intVal: new BigInteger(pubkey_info.parameters.p, 16)
						}, {
							type: jCastle.asn1.tagInteger,
							intVal: new BigInteger(pubkey_info.parameters.q, 16)
						}, {
							type: jCastle.asn1.tagInteger,
							intVal: new BigInteger(pubkey_info.parameters.g, 16)
						}]
					}]
				};
				break;
			...
			case 'ECKCDSA':
				if (jCastle.util.isString(pubkey_info.parameters)) {
					...
					if ('g' in pubkey_info.parameters) {
						var g = pubkey_info.parameters.g;
					} else {
						var g = '04' + pubkey_info.parameters.gx + pubkey_info.parameters.gy;
					}
					...
					var params_schema = {
						...
						items: [{
							...
							type: jCastle.asn1.tagInteger,
							intVal: new BigInteger(pubkey_info.parameters.n, 16)
						}, {
							type: jCastle.asn1.tagInteger,
		...
		switch (pubkey_info.algo) {
			case 'RSA':
				schema.items.push({	
					...
					value: {
						...
						items:[{
							type: jCastle.asn1.tagInteger,
							intVal: new BigInteger(pubkey_info.publicKey.n, 16)
						}, {
							type: jCastle.asn1.tagInteger,
			...
			case 'KCDSA':
				schema.items.push({
					...
					value: {
						type: jCastle.asn1.tagInteger,
						value: new BigInteger(pubkey_info.publicKey, 16)
					}
				});
				break;
...
*/
		...
		if (typeof obj != 'undefined' && obj.type == 0x02) {
			authorityKeyIdentifier.authorityCertSerialNumber = BigInteger.fromByteArray(Buffer.from(obj.value, 'latin1'));
			//authorityKeyIdentifier.authorityCertSerialNumber = new BigInteger(jCastle.encoding.hex.encode(obj.value), 16);
	...
	schema: function(authorityKeyIdentifier)
	{
		...
		if ('authorityCertSerialNumber' in authorityKeyIdentifier) {
			...
			schema.push({
				...
				type: 0x02,
				constructed: false,
	//			value: (typeof serial == 'number' || serial instanceof BigInteger) ? jCastle.encoding.hex.decode(serial.toString(16)) : serial
				value: serial
			});
		}
