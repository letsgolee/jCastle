...
*/
		...
		if (contentType in jCastle.cms.contentType) {
			var cmsInfo = jCastle.cms.contentType[contentType].parse(cmsSequence, options);
		} else {
...
//				indefiniteLength: true,
		...
		if (signerInfos.length == 1 && 'cmsKey' in options) {
			var cmsKeys = [options.cmsKey];
		} else {
			var cmsKeys = options.cmsKeys;
		}
		...
		if ('content' in options) var encapContentInfo = {
			content: options.content,
			contentType: options.contentType || 'data'
		};
		else var encapContentInfo =  cmsInfo.content.encapContentInfo;
		...
		if ('certificates' in options) var certificates = options.certificates;
		else var certificates = cmsInfo.content.certificates;

		if ('crls' in options) var crls = options.crls;
		else var crls = cmsInfo.content.crls;
...
//		var crls = 'crls' in options ? options.crls : ('crls' in originatorInfo ? originatorInfo.crls : []);
		...
		if ('encryptKey' in options) {
			var encryptKey = Buffer.from(options.encryptKey, 'latin1');
			...
			var encryptKey = prng.nextBytes(key_size);
		}
...
//		var crls = 'crls' in options ? options.crls : ('crls' in originatorInfo ? originatorInfo.crls : []);
		...
		if (encapContentInfo.contentType != 'data') {
			if ('digestAlgorithm' in cmsInfo.content) {
				var digestAlgorithm = cmsInfo.content.digestAlgorithm;
			} else {
				if ('hmac' == macAlgorithm.macInfo.type) var digestAlgorithm = macAlgorithm.macInfo.algo;
...
//console.log(keylen);
	...
	switch (keyEncryptionAlgorithm.keyAgreement) {
		...
		case 'dhSinglePass-stdDH':
			...
			var cofactor = calc_cofactor ? jCastle.util.toBigInteger(ecdsa.getCurveInfo().curve.getH()) : null;
			var zz = ecdh.calculateAgreement(other_pubkey, cofactor);
			break;
		case 'mqvSinglePass':
			...
			var zz = ecdh.calculateMQVAgreement(ephemeral_prvkey, other_pubkey, ephemeral_other_pubkey);
			break;
		default: 
...
jCastle.cms.fn.generateAuthMAC = function(content, contentType, authAttrs, macAlgorithm, mackey)
{
	if (authAttrs && authAttrs.length) {
		var attrsSchema = jCastle.cms.asn1.attrs.schema(authAttrs, null);
		var digestInput = jCastle.asn1.create().getDER(attrsSchema);
	} else {
		var digestInput = content;
	}
...
//			signedAttrsDer = obj.der;
		...
		if (obj.type == jCastle.asn1.tagSequence) {
			...
			var signatureAlgorithm = jCastle.certificate.asn1.encryptionInfo.parse(obj);
			obj = sequence.items[idx++];
		}
...
*/
		...
		if (jCastle.util.isString(encapContentInfo.content)) var eContent = encapContentInfo.content;
		else var eContent = jCastle.cms.content.getDER(encapContentInfo.content);
		...
		if ('privateKey' in cmsKey) {
			...
			try {
				...
				if (typeof privkey == 'object' && 'algo' in privkey && 'privateKey' in privkey) {// privateKeyInfo
					...
					var pki = new jCastle.pki(info.keyEncryptionAlgorithm.algo);
...
*/
		...
		if ('certificate' in cmsKey.recipient) {
			...
			var recipientIdentifier = recipientInfo.recipientIdentifier;
			...
			var pki = new jCastle.pki(recipientInfo.keyEncryptionAlgorithm.algo);
...
//console.log(info);
		...
		//     ephemaral private key if the agreement is mqvSinglePass-sha*kdf-scheme
			...
			if (!('publicKey' in originator)) {
				...
				var originator_pubkey = pubkey_info.publicKey;
			} else {
				var originator_pubkey = originator.publicKey;
			}
...
//		throw jCastle.exception("UNSUPPORTED_CMS_STRUCTURE", 'CMS028');
		...
		if ('certificate' in cmsKey) {
			...
			if (jCastle.util.isString(cmsKey.privateKey)) {
				var pki = jCastle.pki.create().parsePrivateKey(cmsKey.privateKey, cmsKey.password);
			} else if (cmsKey.privateKey instanceof jCastle.pki) {
				var pki = options.privateKey;
		...
		if (('ukm' in recipientInfo && recipientInfo.ukm.length) || 'ukm' in options) {
			var ukm = recipientInfo.ukm || options.ukm;
			var ukmSchema = {
				tagClass: jCastle.asn1.tagClassContextSpecific,
				type: 0x01,
				constructed: true,
			...
			var ukm = recipientInfo.mqvukm.addedukm || options.ukm || '';
...
*/
		...
		if ('recipient' in cmsKey) {
			var recipients = [cmsKey.recipient];
		} else {
			var recipients = cmsKey.recipients;
		}
...
*/
		...
		if (obj.tagClass == jCastle.asn1.tagClassContextSpecific) {
			...
			var kdfInfo = jCastle.pbe.asn1.pbkdf2.parse(obj);
...
//			indefiniteLength: true
		...
		if (is_pbe) {
			...
			var encAlgoSchema = jCastle.pbe.asn1.encAlgoInfo.schema(enc_algo_info);
		}
