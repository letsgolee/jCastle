...
jCastle.dh = class
{
	...
	computeSecret(other_pubkey, additional)
	{
		if (typeof other_pubkey == 'object' && 'kty' in other_pubkey && other_pubkey.kty == 'ElGamal') {
			var y = BigInt.fromBufferUnsigned(Buffer.from(other_pubkey.y.replace(/[ \t\r\n]/g, ''), 'base64url'));
			return this.computeSecret(y, additional);
		}

		other_pubkey = jCastle.util.toBigInt(other_pubkey);
		...
		if (additional) {
			additional = jCastle.util.toBigInt(additional);
			this.secret = this.secret.modPow(additional, params.p);
		}

		var secret = this.secret.toBuffer();
