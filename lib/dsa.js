...
jCastle.pki.dsa = class
{
	...
	publicKeyEquals(pubkey)
	{
		...
		if (typeof pubkey == 'object' && 'kty' in pubkey && pubkey.kty == 'DSA') {
			var y = BigInteger.fromByteArrayUnsigned(Buffer.from(pubkey.y.replace(/[ \t\r\n]/g, ''), 'base64url'));
...
*/
		...
		// var hash_bi = BigInteger.fromByteArrayUnsigned(hash.slice(0, this.params.q.bitLength() >>> 3));
...
*/
		...
		if (r.compareTo(zero) <= 0 || r.compareTo(this.params.q) >= 0 ||
...
jCastle.pki.dsa.generateParameters = function(options = {})
{
	...
	for (counter = 0; counter < limit; counter++) {
		...
		x = BigInteger.fromByteArrayUnsigned(U);
	...
	var p1q = p.subtract(one).divide(q);

	do {
		var h = BigInteger.random(pBits, rng);
...
jCastle.pki.dsa.generateParametersExt = function(options = {})
{
	...
	for (counter = 0; counter < limit; counter++) {
		...
		var x = BigInteger.fromByteArrayUnsigned(w);
		var c = x.mod(q.multiply(two));
		var p = x.subtract(c.subtract(one));
