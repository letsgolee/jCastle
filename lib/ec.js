...
//
	...
	isZero()
	{
		return this.toBigInteger().equals(BigInteger.ZERO);
	}
	...
	{
		var x = this;
		var one = new jCastle.math.ec.fieldElement.fp(this.q, BigInteger.ONE);
		var R = one;
	...
	sqrt()
	{
		...
		do {
			var P;
			do {
				//P = new BigInteger(this.q.bitLength(), rng);
                P = BigInteger.random(this.q.bitLength(), rng);
			}
	...
	getByteLength ()
	{
		return (this.toBigInteger().bitLength() + 7) >>> 3;
	}
}
...
jCastle.math.ec.fieldElement.f2m = class
{
    constructor(m, k1, k2, k3, x)
    {
        ...
        if (k2 instanceof BigInteger && typeof k3 == 'undefined') {
	...
	multiply(b)
	{
		...
		var az = this.toBigInteger();
		var bz = b.toBigInteger();
		var cz;
	...
	invert()
	{
		...
		// v(z) := f(z)
		var vz = BigInteger.ZERO.setBit(this.m);
		vz = vz.setBit(0);
		...
		// g1(z) := 1, g2(z) := 0
		var g1z = BigInteger.ONE;
		var g2z = BigInteger.ZERO;
	...
	{
		var x = this;
		var one = new jCastle.math.ec.fieldElement.f2m(this.m, this.k1, this.k2, this.k3, BigInteger.ONE);
		var R = one;
		var bitlen = n.toString(2).length;
...
jCastle.math.ec.point.multiplyPositive = function(point, k, method)
{
	switch (method) {
		...
		case 'classic-double-add':
			var h = k.multiply(BigInteger.valueOf(3));
			var neg = point.negate();
			var R = point;
			var hBit, kBit;
	...
	*/
	...
	encodePoint(compressed = false)
	{
		...
		if(compressed) {
			...
			var encY = y.toByteArrayUnsigned();
	...
	{
		//var len = this.getCurve().getQ().toByteArray().length;
	...
	decodePoint(curve, s)
	{
        ...
        if (type === 0x04) {
			...
			var bit0 = y.toBigInteger().testBit(0) ? 1 : 0;
            var p;
	...
	//				null,
				...
				p = new jCastle.math.ec.point.fp(
					this,
					x,
	//				new jCastle.math.ec.fieldElement.fp(this.q, this.q.subtract(y.toBigInteger())),
					y.negate(),
	//				null,
					true
				);
	...
	validate()
	{
		...
		if (y.compareTo(BigInteger.ONE) < 0 ||
			y.compareTo(q.subtract(BigInteger.ONE)) > 0
		) {
...
jCastle.math.ec.point.f2m = class
{
	...
	encodePoint(compressed)
	{
		...
		var X = this.getX().toBigInteger().toByteArrayUnsigned();
		var Y = this.getY().toBigInteger().toByteArrayUnsigned();
	...
	decodePoint(curve, s)
	{
		...
		// Prepend zero byte to prevent interpretation as negative integer
		// Convert to BigIntegers
		var x = BigInteger.fromByteArrayUnsigned(xBa);
		var y = BigInteger.fromByteArrayUnsigned(yBa);
...
jCastle.math.ec.curve.fp = class
{
	...
	{
		return new jCastle.math.ec.point.fp(this, this.fromBigInteger(x), this.fromBigInteger(y), compressed);
	}
	...
	encodePoint(p)
	{
		...
		var x = p.getX().toBigInteger().toByteArrayUnsigned();
		var y = p.getY().toBigInteger().toByteArrayUnsigned();
	...
	decodePoint(enc)
	{
		...
		switch (s[0]) {
			...
			case 0x03:
                ...
                var x = new jCastle.math.ec.fieldElement.fp(this.q, BigInteger.fromByteArrayUnsigned(s.slice(1)));
				// y^2 = x^3 + a * x + b
				// y^2 = x * (x^2 + a) + b
	...
	//					null,
			...
			case 0x07:
				...
				p = new jCastle.math.ec.point.fp(this,
						this.fromBigInteger(BigInteger.fromByteArrayUnsigned(x)),
						this.fromBigInteger(BigInteger.fromByteArrayUnsigned(y))
					);
				break;
...
//	}
	...
	//		(this.a.toBigInteger().equals(BigInteger.ZERO) || this.a.toBigInteger().equals(BigInteger.ONE)) &&
	//		this.b.toBigInteger().equals(BigInteger.ONE);
	//
		return (this.a.toBigInteger().equals(BigInteger.ZERO) || this.a.toBigInteger().equals(BigInteger.ONE)) &&
			this.b.toBigInteger().equals(BigInteger.ONE);
	...
	{
		var x = this.fromBigInteger(BigInteger.fromByteArrayUnsigned(s));
		var y = null;
		var one = this.fromBigInteger(BigInteger.ONE);
	...
	{
		var zeroElement = this.fromBigInteger(BigInteger.ZERO);
		...
		do {
			var t = this.fromBigInteger(BigInteger.random(this.m, rng));
			z = zeroElement;
			var w = beta;
			for (var i = 1; i < this.m; i++) {
				var w2 = w.square();
...
jCastle.math.ec.curve.f2m.tnaf = {
	...
	getSi: function(fieldSize, curve, cofactor)
	{
		if (typeof curve == 'undefined') {
			...
			var a = curve.getA().toBigInteger().intValue();
	...
	{
		if (h != null) {
			if (h.equals(BigInteger.valueOf(2))) {
				return 1;
			}
			if (h.equals(BigInteger.valueOf(4))) {
				return 2;
			}
		}
