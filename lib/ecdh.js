...
jCastle.ecdh = class
{
	...
	computeSecret(party_pubkey, additional)
	{
		...
		if (additional) {
			var extra;
			if (additional instanceof BigInteger) {
				extra = additional;
			} else if (jCastle.util.isString(additional) && /^[0-9A-F]+$/i.test(additional)) {
				extra = BigInteger(additional, 16);
			} else {
				extra = BigInteger.fromByteArrayUnsigned(Buffer.from(additional, 'latin1'));
			}
...
*/
	...
	calculateAgreement(party_pubkey, additional)
	{
		...
		var bi = point.getX().toBigInteger();
		var ecInfo = this.pki.getCurveInfo();
		var bl = (ecInfo.n.bitLength() + 7) >>> 3;

		var res = Buffer.from(bi.toByteArray());
...
*/
	...
	calculateMQVAgreement(ephemeral_prvkey, party_pubkey, ephemeral_party_pubkey)
	{
		...
		return Buffer.from(bi.toByteArray());
	}
};
