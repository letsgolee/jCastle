...
jCastle.pki.eckcdsa = class
{
	...
	sign(str, options = {})
	{
		...
		for (;;) {
			if (random_k) {
				...
				do {
					k = BigInt.random(this.ecInfo.n.bitLength(), rng, true);
				} while (k.compareTo(0n) <= 0 || k.compareTo(this.ecInfo.n.subtract(1n)) >= 0);
			}

			// (x1, y1) = kG
			w = this.ecInfo.G.multiply(k);
			//r = new jCastle.digest(hash_algo).digest(w.getX().toBigInt().toBufferUnsigned());
			var w_x = w.encodePoint(true).slice(1);
			r = new jCastle.digest(hash_algo).digest(w_x);
			r_i = BigInt.fromBufferUnsigned(r);
			...
			while(t.compareTo(0n) <= 0) t = t.add(this.ecInfo.n);
	...
	verify(str, signature, options = {})
	{
		...
		//var v = new jCastle.digest(hash_algo).digest(w.getX().toBigInt().toBufferUnsigned());
		var w_x = w.encodePoint(true).slice(1);
