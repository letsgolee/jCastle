...
jCastle.pki.elGamal = class
{
	...
	setPublicKey(y, params)
	{
		if (!params && typeof y == 'object' && 'kty' in y && y.kty == 'ElGamal') {
			...
			var yy = BigInt.fromBufferUnsigned(Buffer.from(y.y, 'base64url'));
	...
	setPrivateKey(x, y, params)
	{
		if (typeof y == 'undefined' && typeof x == 'object' && 'kty' in x && x.kty == 'ElGamal') {
			...
			var xx = BigInt.fromBufferUnsigned(Buffer.from(x.x, 'base64url'));
			var yy = BigInt.fromBufferUnsigned(Buffer.from(x.y, 'base64url'));
	...
	publicKeyEquals(pubkey)
	{
		...
		if (typeof pubkey == 'object' && 'kty' in pubkey && pubkey.kty == 'ElGamal') {
	...
	*/
	...
	publicEncrypt(str, options = {})
	{
		...
		pubkey = jCastle.util.toBigInt(pubkey);

		ba = Buffer.from(str);

		var m_bi = BigInt.fromBufferUnsigned(ba);
		
		if (m_bi.compareTo(this.params.p.subtract(1n)) >= 0) {
		...
		if ('kRandom' in options) {
			...
			if (!BigInt.is(k)) {
				if (!Buffer.isBuffer(k) && /^[0-9A-F]+$/i.test(k)) {
					k = BigInt.fromBufferUnsigned(Buffer.from(k, 'hex'));
				} else {
					k = BigInt.fromBufferUnsigned(Buffer.from(k));
				}
			}
		} else {
			do {
				k = BigInt.random(this.params.p.bitLength(), rng);
			} while (k.compareTo(1n) <= 0 || k.compareTo(this.params.p.subtract(1n)) <= 0);
		}
	...
	*/
	...
	privateDecrypt(str, options = {})
	{
		...
		var m_bi = c1.modPow(this.params.p.subtract(this.privateKey).subtract(1n), this.params.p).multiply(c2).mod(this.params.p);
	...
	generateKeypair(parameters)
	{
		...
		var x;
		do {
			x = BigInt.random(this.params.p.bitLength(), rng);
		} while (x.compareTo(0n) <= 0 || x.compareTo(this.params.p.subtract(1n)) >= 0);
	...
	*/
	...
	sign(str, options = {})
	{
		...
		var p1 = this.params.p.subtract(1n);
		var k, t, r, s, res;
		...
		for(;;) {
			if (random_k) {
				...
				// Choose a random k such that 1 < k < p - 1 and gcd(k, p - 1) = 1.
				do {
					k = BigInt.random(this.params.p.bitLength(), rng, true);
				} while (k.compareTo(0n) <= 0 || k.compareTo(p1) >= 0);
			}
	...
	*/
	...
	verify(str, signature, options = {})
	{
		...
		// 0 < r < p and 0 < s < p-1
		if (r.compareTo(0n) <= 0 || r.compareTo(this.params.p) >= 0 ||
			s.compareTo(0n) <= 0 || s.compareTo(p1) >= 0
		) {
			return false;
		}
...
jCastle.pki.elGamal.generateParameters = function(options = {})
{
	...
	do {
		g = BigInt.random(bits, rng);
	}
	while (g.compareTo(p.subtract(1n)) >= 0 || g.compareTo(1n) <= 0);
