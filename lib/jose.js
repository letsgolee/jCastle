...
jCastle.jose.fn.buildOtherInfo = function(algo_id, pu_info, pv_info, pub_info, priv_info)
{
	...
	if (!Buffer.isBuffer(algo_id)) algo_id = Buffer.from(algo_id);

	if (!pu_info) pu_info = Buffer.alloc(0);
	if (!pv_info) pv_info = Buffer.alloc(0);
	...
	switch (algo_info.algoName) {
		...
		case 'ECDH':
			...
			if ('wrapAlgo' in algo_info) {
				...
				// gcm mode has no mackey_size.
				var mackey_size = 'macKeySize' in enc_algo_info ? enc_algo_info.macKeySize : 0;
