...
*/
	...
	sign(message, params, options = {})
	{
		...
		for (var i = 0; i < algo_params.length; i++) {
			...
			var signature = {};
			var header = {};
			var protectContentOnly = 'protectContentOnly' in algo_params[i] && algo_params[i].protectContentOnly ? true : false;
...
*/
	 ...
	 * @returns true if al conditions are met.
	 */
...
*/
			...
			if ('exp' in info.payload && time >= info.payload.exp) { // expires
				console.log('The expiration date has passed. ');
				return false;
			}

			if ('nbf' in info.payload && time < info.payload.nbf) { // not before
...
*/
	...
	encrypt(message, algo_params, enc_algo_params, options = {})
	{
		...
		if (ecdh_direct_key) {
			...
			var cek = jCastle.jose.fn.deriveKeyUsingECDHAndSinglestepKDF(other_pubkey, privkey, apu, apv, enc_algo, cek_length);
		} else {
			var cek;
...
//		}
		...
		//if (nonce_mode.includes(enc_algo_info.mode.toLowerCase())) {
		if (!('macHashAlgo' in enc_algo_info)) {
			var tag = ct.slice(ct.length - enc_algo_info.tagSize);
...
*/
			...
			var encrypted_key = Buffer.from(jwt_arr[1], 'base64url');
			var iv = Buffer.from(jwt_arr[2], 'base64url');
			var ciphertext = Buffer.from(jwt_arr[3], 'base64url');
			var auth_tag = Buffer.from(jwt_arr[4], 'base64url');
			var jose_header = JSON.parse(jwt_header);
			var enc_algo = jose_header.enc;
			var aad = jwt_arr[0];
		} else {
/*
     {
...
*/
			...
			var enc_algo = protected_header.enc;
			...
			var encrypted_key = 'encrypted_key' in recipient ? Buffer.from(recipient.encrypted_key, 'base64url') : '';
			var iv = Buffer.from(jwt.iv, 'base64url');
			var ciphertext = Buffer.from(jwt.ciphertext, 'base64url');
			var aad = 'aad' in jwt ? jwt.aad : null;
			var aad_included = aad ? true : false;
			var auth_tag = Buffer.from(jwt.tag, 'base64url');
	...
	switch (algo_info.algoName) {
		...
		case 'ECDH':
			...
			if ('wrapAlgo' in algo_info) {
				...
				var cek = jCastle.jose.fn.deriveKeyUsingECDHAndSinglestepKDF(
					other_pubkey_jwk,
					privkey_jwk,
					apu, apv,
					jose_header.enc,
