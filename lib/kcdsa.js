...
jCastle.pki.kcdsa = class
{
	...
	//	if (!z) {
	//		z = this.publicKey.mod(1n.shiftLeft(this.params.l));
	//	}
	//	this.z = jCastle.util.toBigInt(z);
	...
	publicKeyEquals(pubkey)
	{
		...
		if (typeof pubkey == 'object' && 'kty' in pubkey && pubkey.kty == 'KCDSA') {
...
*/
	 ...
	 *        {number} pBits p bits in number
	 *        {number} qBits q bits in number
	 *        {string} hashAlgo hash algorithm name
	...
	generateParameters(options)
	{
		...
		var params = jCastle.pki.kcdsa.generateParameters(options);
		this.params = params;
	...
	generateKeypair(parameters)
	{
		...
		do {
//			x = BigInt.probablePrime(this.params.q.bitLength(), rng, certainty);
//			if (!x.isLucasLehmerPrime()) continue;
	...
	////if (this.l) {
	//		this.z = this.y.mod(1n.shiftLeft(this.l));
	////}
	...
	*/
	...
	sign(str, options = {})
	{
		...
		var z = this.publicKey.mod(1n.shiftLeft(l)).toBufferUnsigned();
		// var l = jCastle.digest.getBlockSize(hash_algo);
		// var z = Buffer.alloc(l);
		// var y = this.publicKey.toBuffer();
		...
		for (;;) {
			...
			r = new jCastle.digest(hash_algo).digest(w.toBufferUnsigned());
			r_bi = BigInt.fromBufferUnsigned(r);
	...
	*/
	...
	verify(str, signature, options = {})
	{
		...
		if (typeof signature === 'object' && 'r' in signature && 's' in signature) {
			...
			if (jCastle.asn1.isAsn1Format(signature)) {
				try {
					...
					r = BigInt.fromBufferUnsigned(Buffer.from(sequence.items[0].value, 'latin1'));
					s = sequence.items[1].intVal;
				} catch (ex) {
					// concat
					r = BigInt.fromBufferUnsigned(signature.slice(0, hash_size));
					s = BigInt.fromBufferUnsigned(signature.slice(hash_size));
				}
			} else {
				// concat
				r = BigInt.fromBufferUnsigned(signature.slice(0, hash_size));
		...
		if (r.compareTo(0n) <= 0 || r.compareTo(this.params.q.multiply(2n)) >= 0 ||
		...
		var v = new jCastle.digest(hash_algo).digest(w.toBufferUnsigned());
...
jCastle.pki.kcdsa.generateParameters = function(options = {})
{
	...
	for (;;) {
		if (g_hash) {
			...
			U = BigInt.fromBufferUnsigned(g_hash);
			g_counter++;
		} else {
			U = BigInt.random(pBits, rng);
			if (U.compareTo(1n) <= 0 || U.compareTo(p.subtract(1n) >= 0)) {
				continue;
			}
		}
...
jCastle.pki.kcdsa.generateParametersExt = function(options = {})
{
	...
	do {
		...
		j = BigInt.probablePrime(pBits - qBits - 4, rng, certainty);
	...
	do {
		h = BigInt.random(pBits, rng);
		if (h.compareTo(p.subtract(1n)) >= 0 || h.compareTo(1n) <= 0) {
			continue;
		}

		g = h.modPow(j, p);
	} while (!g || g.compareTo(1n) <= 0);	// sometimes g is undefined...
