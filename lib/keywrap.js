...
*/
				...
				var output = this._algorithm.encryptBlock(Buffer.concat([iv, blocks[0]]));
			} else {
				for (var i = 0; i <= 5; i++) {
					for (var j = 0; j < n; j++) {
						var block = blocks[j];
				...
				var output = Buffer.alloc(block_size * blocks.length);
				var pos = 0;
...
*/
				...
				var output = Buffer.alloc(block_size * (blocks.length - 1));
				var pos = 0;
...
*/
			...
			if (this._options.iv) var iv = Buffer.from(this._options.iv);
			else {
				var prng = jCastle.prng.create();
				var iv = prng.nextBytes(8);
			}
...
*/			
			...
			if (this._options.iv) var iv = Buffer.from(this._options.iv);
				...
				var iv = prng.nextBytes(block_size);
			}
