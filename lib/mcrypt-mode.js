...
*/
			if (lastLength == this.blockSize) {
				...
				var output = this.type == 'CS3' ? Buffer.concat([lastBlock, block]) : Buffer.concat([block, lastBlock]);
			} else {
				...
				if (this.type == 'CS2' || this.type == 'CS3') {
					var output = Buffer.concat([lastBlock, block.slice(0, lastLength)]);
				} else {
					var output = Buffer.concat([block.slice(0, lastLength), lastBlock]);
				}				
			}
		} else {
/*
...
*/
			if (lastLength == this.blockSize) {
				...
				if (this.type == 'CS2' || this.type == 'CS3') {
					...
					var output = Buffer.concat([block, lastBlock.slice(0, lastLength)]);
				} else {
...
*/
			...
			if (this.dataUnitLength) {
				var bits = this.bitsLength - this.dataUnitLength;
				if (bits < 0) {
			...
			var prevBlock = Buffer.xor(this.state, this.tweak);
			...
			if (lastLength == this.blockSize) {
				...
				var lastBlock = Buffer.slice(prevBlock, 0, lastLength);
			...
			if (lastLength == this.blockSize) {
				if (!stealBits) { // two blocks of full 128 bits
					...
					var lastBlock = Buffer.xor(this.lastBlock, this.tweak);
					...
					var lastBlock = Buffer.slice(this.lastBlock);
					var stealedBits = this.stealBits(lastBlock, lastLength - 1, bits);
...
*/
	...
	finish()
	{
		...
		if (this.isEncryption) {
			if (this.state.length) {
				var output = this.ctrMode.process(this.state);
				this.cMac.process(output);
			}
...
jCastle.mcrypt.mode['gcm'] = class
{
	...
	init(algorithm, options)
	{
		...
		// this.R = new BigInteger("11100001", 2).shiftLeft(120);
...
*/
	...
	process(input)
	{
		if (this.isEncryption) {
			...
			var output = this.galoisCTR(temp, size);
			...
			if (input.length < this.blockSize) {
				...
				var output = this.galoisCTR(temp, input.length);
				this.state = Buffer.concat([this.state.slice(input.length), input]);
				this.dataLength += output.length;
				return output;
			}
			// normall
			var output = this.galoisCTR(this.state, input.length);
			this.state = Buffer.slice(input);
			this.dataLength += output.length;
			return output;
		}
...
jCastle.mcrypt.mode['gcm'] = class
{
	...
	init(algorithm, options)
	{
		...
		if (typeof BigIntegr == 'undefined') {
			throw jCastle.exception("BIGINTEGER_REQUIRED", 'MOD037');
		}
		...
		if (this.state.length == 12) {
			...
			var X = BigInteger.valueOf(this.state.length * 8);
			N = this.gmultiply(N.xor(X), this.H);
			this.J0 = this.BigIntegerToBuffer(N, this.blockSize);
        }
	...
	finish()
	{
		...
		var X = BigInteger.valueOf(this.additionalData.length * 8).shiftLeft(64).add(BigInteger.valueOf(data_length * 8));
		...
		var T = this.S.xor(new BigInteger(tBytes));
		var tag = this.BigIntegerToBuffer(T, this.blockSize);
	...
	ghashBlock(block)
	{
		var X = block instanceof BigInteger ? block : new BigInteger(block);
		this.S = this.gmultiply(this.S.xor(X), this.H);
	}
}
*/
...
/*
	...
	{
		if (typeof BigIntegr == 'undefined') {
			throw jCastle.exception("BIGINTEGER_REQUIRED", 'MOD047');
		}

		var z = BigInteger.fromByteArray(hashkey); // 34ae6a6fe9517894acccbb9ebae7208c
		//var m = BigInteger.valueOf(2).pow(127).subtract(BigInteger.One); // 2^127 - 1
		var m = new BigInteger('170141183460469231731687303715884105727');
		//var n = BigInteger.valueOf(2).pow(64); // 2^64
		var n = new BigInteger('18446744073709551616');
		...
		while (p < x.length) {
			t = BigInteger.fromByteArrayUnsigned(x.slice(p, p + 12));
			output = output.add(t).mod(m);
			output = output.multiply(z).mod(m);
			p += 12;
		}

		t = BigInteger.valueOf(ciphertext.length);
		t = t.add(n.multiply(BigInteger.valueOf(additionalData.length)));

		output = output.add(t).mod(m);

		return Buffer.from(output.toByteArray());
	}
*/
	cwcHash(hashkey, additionalData, ciphertext)
	{
...
//		}
	...
	{
		if (this.isEncryption) {
			var output = this.algorithm.encryptBlock(Buffer.alloc(input.length));
			...
			var output = this.algorithm.decryptBlock(Buffer.alloc(this.state.length));
	...
	{
		if (this.isEncryption) {
			var mac = this.calculateMac(this.polyKey, this.additionalData, this.cdata);
			...
			var mac = Buffer.slice(this.state, this.state.length - 16);
