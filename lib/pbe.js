...
*/
	encrypt: function(pbe_info, password, data)
	{
		...
		if (algo_info.algo == 'seed' || algo_info.algo == 'seed-128') {
			...
			pbdk_size = key_size + algo_info.blockSize;
		}
		...
		if (algo_info.algo == 'seed' || algo_info.algo == 'seed-128') {
			if (algo_info.oid == '1.2.410.200004.1.4') {
				iv = Buffer.from('123456789012345', 'latin1');
			} else {
				var div = Buffer.slice(pbdk, key_size);
				var md = new jCastle.digest(algo_info.hashAlgo);
				div = md.digest(div);
				iv = Buffer.slice(div, 0, algo_info.blockSize);
			}
		} else {
			iv = Buffer.slice(pbdk, algo_info.keySize, algo_info.keySize + algo_info.blockSize);
		}
...
//console.log(algo_info);
		...
		if (algo_info.algo == 'seed' || algo_info.algo == 'seed-128') {
			...
			dk_size = key_size + algo_info.blockSize;
		}
...
//				iv = jCastle.mcrypt.createInitialVector(algo_info.blockSize);
		...
		if (!salt) {
			salt_len = salt_len || 8;
			salt = prng.nextBytes(salt_len, true);
			jCastle.util.avoidAsn1Format(salt);
		}
...
*/
	parse: function(sequence)
	{
		...
		if (typeof sequence.items[1].items[idx] != 'undefined' &&
			...
			// key_size = jCastle.util.str2int(sequence.items[1].items[idx++].value);
			key_size = sequence.items[1].items[idx++].intVal;
		}
			...
			case 'cbc':
				if (enc_algo_info.algoInfo.algo == 'rc2') {
					...
					if ('effectiveKeyBits' in enc_algo_info.params) {
						rc2_pversion = jCastle.algorithm.rc2.getVersion(enc_algo_info.params.effectiveKeyBits);
					} else {
						var key_size = 'keySize' in enc_algo_info.params ? enc_algo_info.params.keySize : enc_algo_info.algoInfo.keySize;
						rc2_pversion = jCastle.algorithm.rc2.getVersion(key_size * 8);
					}
