...
*/
				...
				if (!contentsSequence || contentsSequence.type != jCastle.asn1.tagSequence) {
...
*/
				...
				var contentsSequence = safeContentsSequence.items[1].items[0].value;
			}
...
*/
				var pbe_info = jCastle.pbe.asn1.pbeInfo.parse(safeBagSequence.items[1].items[0].items[0]);
							...
							SEQUENCE(6 elem)
								...
								SET(1 elem)
									SEQUENCE(2 elem)
										...
										...
*/
				var subType = jCastle.oid.getName(safeBagSequence.items[1].items[0].items[0].value);
				var cert = new jCastle.certificate();
				if (subType == 'x509Certificate') {				
					var cert_info = cert.parse(safeBagSequence.items[1].items[0].items[1].items[0].value, 'asn1', jCastle.certificate.typeCRT);
				} else if (subType == 'sdsiCertificate') {
					var der = jCastle.base64.decode(safeBagSequence.items[1].items[0].items[1].items[0].value, true);
					var cert_info = cert.parse(der, 'der', jCastle.certificate.typeCRT);
				} else {
...
*/
				var cert = new jCastle.certificate();
				var cert_info = cert.parse(ssafeBagSequence.items[1].items[0].value, 'asn1', jCastle.certificate.typeCRL);
				bagContent = cert_info;
				break;
			case 'secretBag':
/*
