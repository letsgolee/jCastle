...
*/
	...
	setPublicKey(n, e)
	{
		...
		if (typeof e == 'undefined' && typeof n == 'object') {
			if ('kty' in n && n.kty == 'RSA') { // jwt
				...
				if (arg.n) n = BigInteger.fromByteArrayUnsigned(Buffer.from(arg.n.replace(/[ \t\r\n]/g, ''), 'base64url'));
		...
		pubkey.n = jCastle.util.toBigInteger(n);
		...
		if (typeof e == 'number' && e % 1 === 0) {
			...
			var e_i = jCastle.util.toBigInteger(e);
			pubkey.e = e_i.intValue();
			//throw jCastle.exception("INVALID_PUBKEY", 'RSA001');
		}

		if (pubkey.n instanceof BigInteger) {
	...
	getPublicKey(format = 'object')
	{
		if (this.hasPubKey) {
			...
			switch (format.toLowerCase()) {
				...
				default:
					return {
						n: jCastle.util.formatBigInteger(this.publicKey.n, format),
						e: jCastle.util.formatBigInteger(BigInteger(this.publicKey.e.toString(16), 16), format)
					};
			...
			switch (format.toLowerCase()) {
				...
				default:
					return {
						n: jCastle.util.formatBigInteger(this.publicKey.n, format),
						e: jCastle.util.formatBigInteger(BigInteger(this.publicKey.e.toString(16), 16), format),
						d: jCastle.util.formatBigInteger(this.privateKey.d, format),
						p: jCastle.util.formatBigInteger(this.privateKey.p, format),
						q: jCastle.util.formatBigInteger(this.privateKey.q, format),
						dmp1: jCastle.util.formatBigInteger(this.privateKey.dmp1, format),
						dmq1: jCastle.util.formatBigInteger(this.privateKey.dmq1, format),
						iqmp: jCastle.util.formatBigInteger(this.privateKey.iqmp, format)
					};
			}
		}
	...
	publicKeyEquals(pubkey)
	{
		...
		try {
			var n = jCastle.util.toBigInteger(pubkey.n);
	...
	setPrivateKey(n, e, d, p, q, dmp1, dmq1, iqmp)
	{
		...
		if (typeof e == 'undefined' && typeof n == 'object') {
			if ('kty' in n && n.kty == 'RSA') { // jwt
				...
				if (arg.n) n = BigInteger.fromByteArrayUnsigned(Buffer.from(arg.n.replace(/[ \t\r\n]/g, ''), 'base64url'));
				if (arg.e) e = parseInt(Buffer.from(arg.e.replace(/[ \t\r\n]/g, ''), 'base64url').toString('hex'), 16);
				if (arg.d) d = BigInteger.fromByteArrayUnsigned(Buffer.from(arg.d.replace(/[ \t\r\n]/g, ''), 'base64url'));
				if (arg.p) p = BigInteger.fromByteArrayUnsigned(Buffer.from(arg.p.replace(/[ \t\r\n]/g, ''), 'base64url'));
				if (arg.q) q = BigInteger.fromByteArrayUnsigned(Buffer.from(arg.q.replace(/[ \t\r\n]/g, ''), 'base64url'));
				if (arg.dp) dmp1 = BigInteger.fromByteArrayUnsigned(Buffer.from(arg.dp.replace(/[ \t\r\n]/g, ''), 'base64url'));
				if (arg.dq) dmq1 = BigInteger.fromByteArrayUnsigned(Buffer.from(arg.dq.replace(/[ \t\r\n]/g, ''), 'base64url'));
				if (arg.qi) iqmp = BigInteger.fromByteArrayUnsigned(Buffer.from(arg.qi.replace(/[ \t\r\n]/g, ''), 'base64url'));
				if ('kid' in arg) keyid = arg.kid;
			} else {
				var arg = n;
		...
		if (d) privkey.d = jCastle.util.toBigInteger(d);

		if (typeof p != 'undefined' && typeof q != 'undefined') {
			// p
			privkey.p = jCastle.util.toBigInteger(p);

			// q
			privkey.q = jCastle.util.toBigInteger(q);

			var p1 = privkey.p.subtract(BigInteger.ONE);
			var q1 = privkey.q.subtract(BigInteger.ONE);

			if (!d) {
				var phi = p1.multiply(q1);
				var e_i = BigInteger.valueOf(this.publicKey.e);
			...
			if (typeof dmp1 != 'undefined' && typeof dmq1 != 'undefined' && typeof iqmp != 'undefined') {
				// dmp1
				privkey.dmp1 = jCastle.util.toBigInteger(dmp1);

				// dmq1
				privkey.dmq1 = jCastle.util.toBigInteger(dmq1);

				// iqmp
				privkey.iqmp = jCastle.util.toBigInteger(iqmp);
			} else {
				if (!this.publicKey.n) {
		...
		if (this.publicKey.n instanceof BigInteger) {
	...
	validateKeypair(privkey, certainty = 10, display_err = false)
	{
		...
		if ('kty' in privkey && privkey.kty == 'RSA') { // jwt
			n = BigInteger.fromByteArrayUnsigned(Buffer.from(arg.n, 'base64url'));
			e_i = BigInteger.fromByteArrayUnsigned(Buffer.from(arg.e, 'base64url'));
			e = e_i.intValue();
//			e = parseInt(ByteBuffer.parseBase64(privkey.e, true).toString(16), 16);
//			e_i = new BigInteger(e.toString(16), 16);
			d = BigInteger.fromByteArrayUnsigned(Buffer.from(arg.d, 'base64url'));
			p = BigInteger.fromByteArrayUnsigned(Buffer.from(arg.p, 'base64url'));
			q = BigInteger.fromByteArrayUnsigned(Buffer.from(arg.q, 'base64url'));
			dmp1 = BigInteger.fromByteArrayUnsigned(Buffer.from(arg.dmp1, 'base64url'));
			dmq1 = BigInteger.fromByteArrayUnsigned(Buffer.from(arg.dmq1, 'base64url'));
			iqmp = BigInteger.fromByteArrayUnsigned(Buffer.from(arg.iqmp, 'base64url'));
		} else {
			n = jCastle.util.toBigInteger(privkey.n);
			e_i = jCastle.util.toBigInteger(privkey.e);
			e = e_i.intValue();
			d = jCastle.util.toBigInteger(privkey.d);
			p = jCastle.util.toBigInteger(privkey.p);
			q = jCastle.util.toBigInteger(privkey.q);
			dmp1 = jCastle.util.toBigInteger(privkey.dmp1);
			dmq1 = jCastle.util.toBigInteger(privkey.dmq1);
			iqmp = jCastle.util.toBigInteger(privkey.iqmp);
		}
		...
		if (!p.subtract(BigInteger.ONE).gcd(e_i).isOne()) {
			if (display_err) console.log('gcd(p-1, e) !== 1');
			return false;
		}
		
		if (!q.subtract(BigInteger.ONE).gcd(e_i).isOne()) {
...
*/
	...
	_privateCrypt(x)
	{
		...
		while (h.compareTo(BigInteger.ZERO) <= 0) h = h.add(this.privateKey.p);
...
*/
	...
	_encrypt(is_private, pt, options = {})
	{
		...
		var bi_ba = BigInteger.fromByteArrayUnsigned(ba);

		var c = is_private ? this._privateCrypt(bi_ba) : this._publicCrypt(bi_ba);
		if (c === null) return null;

		ba = Buffer.from(c.toByteArray());
...
*/
	...
	// when a BigInteger starts a digit bigger than or equal 0x80 
	// then it might be treated as a minus number...
	// therefore BigInteger.fromByteArrayUnsigned() must be used.
		...
		var ba = Buffer.from(bi_m.toByteArray());
...
// Generate a new random private key B bits long, using public exponent E
	...
	generateKeypair(options = {})
	{
		...
		//if (jCastle.util.isInteger(exponent)) {
			e = exponent;
			bi_e = new BigInteger(exponent);
		} else {
			bi_e = jCastle.util.toBigInteger(exponent);
		...
		for (;;) {
			...
			p = BigInteger.fromByteArrayUnsigned(T);

			if (!p.isProbablePrime(certainty)) continue;

			if (p.bitLength() != pBits) continue;
			if(p.subtract(one).gcd(bi_e).isOne()) break;
		...
		for (;;) {
			...
			for (;;) {
				...
				q = BigInteger.fromByteArrayUnsigned(T);
				...
				if(q.subtract(one).gcd(bi_e).isOne()) break;
	...
	generateKeypairExt(options = {})
	{
		...
		for(;;) {
			p = BigInteger.probablePrime(pBits, rng, certainty);
			if (p.bitLength() != pBits) continue;

			if(p.subtract(BigInteger.ONE).gcd(bi_e).isOne()) break;
		}
		...
		for (;;) {
			...
			for (;;) {
				...
				if(q.subtract(BigInteger.ONE).gcd(bi_e).isOne()) break;
			}
...
*/
	...
	sign(str, options = {})
	{
		...
		//var em_i = new BigInteger(em);
		var em_i = BigInteger.fromByteArrayUnsigned(em);
		var s_i = this._privateCrypt(em_i);
		var s = Buffer.from(s_i.toByteArray());
	...
	verify(str, signature, options = {})
	{
		...
		if (!Buffer.isBuffer(signature)) {
			if (/^[0-9A-f]+$/i.test(signature)) 
				s_i = BigInteger.fromByteArrayUnsigned(Buffer.from(signature, 'hex'));
			else 
				s_i = BigInteger.fromByteArrayUnsigned(Buffer.from(signature, 'latin1'));
		} else {
			s_i = BigInteger.fromByteArrayUnsigned(signature);
		}
...
*/
	...
	pssSign(str, options = {})
	{
		...
		for (;;) {
			...
			bi = BigInteger.fromByteArrayUnsigned(ba);
		...
		var sig = Buffer.from(sig_bi.toByteArray());
...
*/
	...
	pssVerify(str, signature, options = {})
	{
		...
		var em = Buffer.from(bi_ba.toByteArray());
...
*/
	...
	ansiX931Sign(str, options = {})
	{
		...
		var sign_bi = this._privateCrypt(BigInteger.fromByteArrayUnsigned(pad_ba));
		var sig = Buffer.from(sign_bi.toByteArray());
	...
	ansiX931Verify(str, signature, options = {})
	{
		...
		var em = Buffer.from(bi_em.toByteArray());
