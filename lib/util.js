...
*/
	...
	*/
	...
	// be careful when you try to turn a string to a BigInt.
	//
	// if you try to turn a string to byte array and then make a BigInt using the byte array,
	// then you might have a wrong result when the number of byte array starts with 8 or bigger.
	// it might be that it is treated as a minus number. Cryptography uses only positive number.
	//
	// but if the byte array is turned to hex string and then it will be no problem.
	// 
	// so if you need a BigInt use toBigInt function!
	toBigInt: function(input, encoding = 'latin1')
	{
		var buf;

		if (BigInt.is(input) || typeof input == 'number') {
			return BigInt(input);
		}
		...
		return BigInt.fromBufferUnsigned(buf);
	},

	formatBigInt: function(bi, format = 'latin1')
	{
		switch (format.toLowerCase()) {
			case 'bytearray':
				return Array.prototype.slice.call(bi.toBuffer(), 0);
			case 'uint8':
			case 'uint8array':
				return new Uint8Array(bi.toBuffer());
			case 'int':
			case 'integer':
				return Number(bi);
			case 'object':
			case 'bigint':
				return BigInt(bi);
			default:
				var buf = Buffer.from(bi.toBuffer());
	...
	str2int: function(str)
	{
		...
		var v = BigInt.fromBuffer(Buffer.from(str, 'latin1'));
