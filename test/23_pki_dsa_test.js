...
// https://www.ietf.org/rfc/rfc6979.txt
    ...
    var testVectors = [
        {
            ...
            p: `86F5CA03DCFEB225063FF830A0C769B9DD9D6153AD91D7CE27F787C43278B447
                ...
                881870B7B19B2B58F9BE0521A17002E3BDD6B86685EE90B3D9A1B02B782B1779`.replace(/[ \t\n]/g, ''),
            ...
            g: `07B0F92546150B62514BB771E2A0C0CE387F03BDA6C56B505209FF25FD3C133D
                ...
                17BE5DCE2F40D10A46A3A3943F26AB7FD9C0398FF8C76EE0A56826A8A88F1DBD`.replace(/[ \t\n]/g, ''),
            ...
            y: `5DF5E01DED31D0297E274E1691C192FE5868FEF9E19A84776454B100CF16F653
                ...
                82F65CBDC4FAE93C2EA212390E54905A86E2223170B44EAA7DA5DD9FFCFB7F3B`.replace(/[ \t\n]/g, ''),
        ...
        },
        {
            ...
            p: `9DB6FB5951B66BB6FE1E140F1D2CE5502374161FD6538DF1648218642F0B5C48
                ...
                3E433256A2261C6A06ED3693797E7995FAD5AABBCFBE3EDA2741E375404AE25B`.replace(/[ \t\n]/g, ''),
            ...
            g: `5C7FF6B06F8F143FE8288433493E4769C4D988ACE5BE25A0E24809670716C613
                ...
                DC4473F996BDCE6EED1CABED8B6F116F7AD9CF505DF0F998E34AB27514B0FFE7`.replace(/[ \t\n]/g, ''),
            ...
            y: `667098C654426C78D7F8201EAC6C203EF030D43605032C2F1FA937E5237DBD94
                ...
                74E04299F132026601638CB87AB79190D4A0986315DA8EEC6561C938996BEADF`.replace(/[ \t\n]/g, ''),
    ...
    for (var i = 0; i < testVectors.length; i++) {
        ...
        var dsa = jCastle.pki.dsa.create();

        dsa.setParameters(vector.p, vector.q, vector.g);
        dsa.setPrivateKey(vector.x, vector.y);

        var params = dsa.getParameters('object');
        var privkey = dsa.getPrivateKey('object');
        var pubkey = dsa.getPublicKey('object');
        ...
        for (var j = 0; j < vector.signatures.length; j++) {
            ...
            var message = sig_vector.message;
            var k = Buffer.from(sig_vector.k, 'hex');
            ...
            k = k.modInverse(params.q).multiply(hash_bi.add(privkey.multiply(r)));

            var s = k.mod(params.q);

            assert.ok(v_r.equals(r), 'signature r step test');
            assert.ok(v_s.equals(s), 'signature s step test');
...
QUnit.test("FIPS 186-4 Vector Test", function(assert) {
    ...
    // # Mod sizes selected: L=1024, N=160, SHA-1 L=1024, N=160, SHA-224 L=1024, N=160, SHA-256 L=1024, N=160, SHA-384 L=1024, N=160, SHA-512 
    // L=2048, N=224, SHA-1 L=2048, N=224, SHA-224 L=2048, N=224, SHA-256 L=2048, N=224, SHA-384 L=2048, N=224, SHA-512 L=2048, N=256, SHA-1 
    // L=2048, N=256, SHA-224 L=2048, N=256, SHA-256 L=2048, N=256, SHA-384 L=2048, N=256, SHA-512 L=3072, N=256, SHA-1 L=3072, N=256, SHA-224 
    // L=3072, N=256, SHA-256 L=3072, N=256, SHA-384 L=3072, N=256, SHA-512
    // # Generated on Tue Aug 16 11:21:08 2011

    var testVectors = [
        {
        L: 1024, N: 160, HashAlgo: 'SHA-1',
    ...
    for (var i = 0; i < testVectors.length; i++) {
        ...
        for (var j = 0; j < vector.sigVectors.length; j++) {
            ...
            var k = Buffer.from(sigVector.K, 'hex');

            var msg = Buffer.from(sigVector.Msg, 'hex');
            var v_r = Buffer.from(sigVector.R, 'hex');
            var v_s = Buffer.from(sigVector.S, 'hex');

            var sig = dsa.sign(msg, {
                hashAlgo,
                kRandom: k,
                returnType: 'object'
            });

            // console.log(sig.r.equals(v_r));
            // console.log(sig.s.equals(v_s));

            assert.ok(sig.r.equals(v_r), 'r test ' + (j+1));
            assert.ok(sig.s.equals(v_s), 's test ' + (j+1));

            var v = dsa.verify(msg, sig, {
                hashAlgo
            });
...
QUnit.test("Basic Test", function(assert) {
    ...
    //----------------------------------------------------------------------------------------------------
        ...
        var pem = dsa.exportParameters();
    
        //console.log(pem);
    
        dsa.parseParameters(pem);
    
        var v_pem = dsa.exportParameters();
    
        assert.equal(v_pem, pem, "DSA.exportParameters 1st Test");
    ...
    */
        ...
        var params_pem = "-----BEGIN DSA PARAMETERS-----\n"+
        "MIICLAKCAQEAjXspeahIbDpFfSxWBn0lUuB79GXOUqY0yBqU+QfPWrK8tY1EXgEH\n"+
        "Apd113b+D3ihkzIEtr/jWKWuHI1nBfx7Gu6NaNrfh7TbiFAvYTfXIHkvaRG4edwK\n"+
        "LtFe2fFIPhaqlvtuo04zdhCMdyUcQerY1aOipn5Q4djCYlWvay0pxKV3Q2x/ZE3M\n"+
        "m5wbiQrKcXD8jSzAu979dgrmVDJblx3QarznyzBeclZ7g8RNCdz3r9A6huafQ0Th\n"+
        ...
        dsa.parseParameters(params_pem);
    
        var v_pem = dsa.exportParameters();
    
        assert.equal(v_pem, params_pem, "DSA.exportParameters 2nd Test");
    ...
    
    ...
    */
        ...
        dsa.parsePrivateKey(pem, password);
    
        var priv_pem = dsa.exportPrivateKey({
            password: password,
            algo: 'des-ede3-cbc',
            salt: Buffer.from('34320BCA43CF4F35', 'hex'),
            iv: Buffer.from('1B20F0EAFC41C572', 'hex')
        });
    
        assert.equal(priv_pem, pem, "DSA.parsePrivateKey & exportPrivateKey with password Test");
    ...
    //----------------------------------------------------------------------------------------------------
        ...
        var pem = "-----BEGIN DSA PRIVATE KEY-----\n"+
        "Proc-Type: 4,ENCRYPTED\n"+
        "DEK-Info: CAMELLIA-256-CBC,450803872E44A668B6AF0A33B22B4C6A\n"+
        "\n"+
        "EiWRPwNSJarOD7+KDH+pdnP/5BqHPmdX9Q4JnIFp1fOhRLmIaJEVGGiMsGhqF4UE\n"+
        "moH9C5yJWqKyNO2GAK9260MHlaevSbPFr9XVt/eTBL/Nia6RtuaOI/VPRMefqoFx\n"+
        "B6BJjHEkTR/Lp+6w/9JQWWr7oJrhOQyzArkkZFJTsF+m0TxF5jsLmYM6LEqDtJhL\n"+
        "qb6Rcq9HHeL3pGucN/UtikKACkziRSf5LObFrW8dS3aa4kscpL/bjUoSg4Wy80B2\n"+
        "PDqpOOsTH+v7gEbUJc+RiYGbdqtovXrFI2W4sPPhQq3P2Rbb0WwLFajSURmlIqbU\n"+
        "R/jLfsNShG7ZMzynetdneVyvuoBKLsvKenDgluBWl/hpuFiayop03xrolxFMFd+b\n"+
        "69yycn1HjuoQVJaSUwZIkrjIPg5OFXbMoypCWhLght3j2fvxmnVIgM/2m0TK6Oso\n"+
        "SrG30z/KMnRq7+ZDAAtbYtEHZPN+8fYWOrk5ApYPBQ7EWi9VhdNQu/sST2QGPUb3\n"+
        "cFyAg6m3ow0ZH+/tz4VAf4WSuSWRPrDdL1/re/4f/JGu87rCkJBYVwwqa3Fg+/xi\n"+
        "LEPr/tdkvbtdQRvQsVLsxtjWx4aYdouqFWAhma2vhPC6/IJNIx2fFi0Xm1+8EzV9\n"+
        "Lb2WTe/L4FfBijORTX3hPELjXslR/TCIchW1bjFXSSC+IbEjvH3k9t6l1t7P/8CD\n"+
        "iWdFxbeecUnx/esHFaFIGcV7IRmx2hT0aSdOtdlBU8bIz2gpZ8U6x2RDw7wFKpLT\n"+
        "TdL4rBwj2CIVnTGwCPjevsiL1RPfAruIuR10LXax1vkiidOjq12Sa3Mqv7udi1Il\n"+
        "YhPaoF7JdBv5gW+/JC4MClQP5VZsDG/9RQMZI3T7RgI8QI7ODIdZCNN4fpdE0Fa8\n"+
        "GoiwgOBMdq1o2yvQSOCq3RyqGyGrSs5SWhcktUfa45ZqQ997RMSYEJlEwKr1Uo2s\n"+
        "nKOzikoZzJOPGA5LwENFKdB59HfDY9SO5d5cOD3X1IDhVDCExLdJ03I96bZANA71\n"+
        "FA92GidhnekwL0YuonjEuSz4oSWvZ1QMsYm26gA3rcAthEB8xhtoenk98V71JDCE\n"+
        "BaM6r0kRteXFErROkDk7MK1CYvx1XxL0ChISMIgOvEPxLhnPoT1Q7R2IFOqVNaHP\n"+
        "-----END DSA PRIVATE KEY-----";
    
        dsa.parsePrivateKey(pem, password);
    
        var priv_pem = dsa.exportPrivateKeyPKCS5({
            password: password,
            algo: 'camellia-256',
            mode: 'CBC',
            iv: Buffer.from('450803872E44A668B6AF0A33B22B4C6A', 'hex')
        });
    
        assert.equal(priv_pem, pem, "DSA.parsePrivateKey & exportPrivateKeyPKCS5 with password Test");
    ...
    //----------------------------------------------------------------------------------------------------
        ...
        var pem = "-----BEGIN DSA PRIVATE KEY-----\n"+
        "MIIDVQIBAAKCAQEAjXspeahIbDpFfSxWBn0lUuB79GXOUqY0yBqU+QfPWrK8tY1E\n"+
        "XgEHApd113b+D3ihkzIEtr/jWKWuHI1nBfx7Gu6NaNrfh7TbiFAvYTfXIHkvaRG4\n"+
        "edwKLtFe2fFIPhaqlvtuo04zdhCMdyUcQerY1aOipn5Q4djCYlWvay0pxKV3Q2x/\n"+
        "ZE3Mm5wbiQrKcXD8jSzAu979dgrmVDJblx3QarznyzBeclZ7g8RNCdz3r9A6huaf\n"+
        "Q0ThCAxUrna1fd/6lxojs5RCYV5m83oa2y4lYOCGtviHeOQmY3WnoiMqqKVG3/OY\n"+
        "n9HlOE1GcTszJwNaa/no4cKalToAxJW7F6KciQIhAJUGNCdjxzgA7i25OP0Ere60\n"+
        "VP+KO5HPwUVRbKtBuRr/AoIBAEmkMXEAp/YvCh70savzWno7Tq5tGsq4eU3w6Q12\n"+
        "a1GKOdrR7AS4dByaUfLCo3zdIuY8/LRLzn+61RMHNbUXh2Fmt4LCVwTRmpCI9Ivx\n"+
        "7laDZ9qXLoAadFRBr3Bx3qrgIyaHskJBSsqzG8lkdKuf6hhM1M/eNtQsrF9R1sck\n"+
        "+5POWqFYvD2PKNeuhtXNNReYE4PJmr1XmaNVWIrzr7z62qgeC3sEkFbBrm4gQjww\n"+
        "mBcZgD4yM20m6qm2ZvrOipwsXMwkEmP5oc3BRZyLiT36CBgVvskj9ayR1y0erFrq\n"+
        "XltPCyzi8JXyf8HgjQcMrhLbBO3Bn0G6eQBP/MQiLEukYAwCggEAYG4tZXyIcVA3\n"+
        "yjpgcw4Uq/i7ZDD04XSqUppbgcGh66Q7aIF7ibz7r1Yco2BomjqMwtxL4WsBM7D/\n"+
        "v/bBut7HbFlBh8XSjDGhAEg2DCF+YsFxcQsZk25Id3q+hEJ3+ylp80PFI9u8jGpa\n"+
        "UHtfdrnpNW79hx1AoaeNZAte8sS1bh9qkYBWrCaGaC4r9EERiogklDbB6X7/D+Nz\n"+
        "a0RJxEn2gifEarB6ml1u/HzkATgJqD/7wSMzdvFtbpNOkeLr8cKv2cRPusRCUWj4\n"+
        "UgeD0KeBNb3Qk3jIhg96jhBWIJOvvaF002OzjhDT3Kpb2+iAkF88XKyrCExCaz7F\n"+
        "sBixIRQy7wIgZneAO/vIGZcjK58ngfaUoYrH8psdfOuFKywDZjrbgfA=\n"+
        "-----END DSA PRIVATE KEY-----";
    
        dsa.parsePrivateKey(pem);
    
        var priv_pem = dsa.exportPrivateKeyPKCS5();
    
        assert.equal(priv_pem, pem, "DSA.parsePrivateKey & exportPrivateKeyPKCS5 no password Test");
    ...
    //----------------------------------------------------------------------------------------------------
        ...
        "MIIDRjCCAjkGByqGSM44BAEwggIsAoIBAQCNeyl5qEhsOkV9LFYGfSVS4Hv0Zc5S\n"+
        "pjTIGpT5B89asry1jUReAQcCl3XXdv4PeKGTMgS2v+NYpa4cjWcF/Hsa7o1o2t+H\n"+
        "tNuIUC9hN9cgeS9pEbh53Aou0V7Z8Ug+FqqW+26jTjN2EIx3JRxB6tjVo6KmflDh\n"+
        "2MJiVa9rLSnEpXdDbH9kTcybnBuJCspxcPyNLMC73v12CuZUMluXHdBqvOfLMF5y\n"+
        "VnuDxE0J3Pev0DqG5p9DROEIDFSudrV93/qXGiOzlEJhXmbzehrbLiVg4Ia2+Id4\n"+
        "5CZjdaeiIyqopUbf85if0eU4TUZxOzMnA1pr+ejhwpqVOgDElbsXopyJAiEAlQY0\n"+
        "J2PHOADuLbk4/QSt7rRU/4o7kc/BRVFsq0G5Gv8CggEASaQxcQCn9i8KHvSxq/Na\n"+
        "ejtOrm0ayrh5TfDpDXZrUYo52tHsBLh0HJpR8sKjfN0i5jz8tEvOf7rVEwc1tReH\n"+
        "YWa3gsJXBNGakIj0i/HuVoNn2pcugBp0VEGvcHHequAjJoeyQkFKyrMbyWR0q5/q\n"+
        "GEzUz9421CysX1HWxyT7k85aoVi8PY8o166G1c01F5gTg8mavVeZo1VYivOvvPra\n"+
        "qB4LewSQVsGubiBCPDCYFxmAPjIzbSbqqbZm+s6KnCxczCQSY/mhzcFFnIuJPfoI\n"+
        "GBW+ySP1rJHXLR6sWupeW08LLOLwlfJ/weCNBwyuEtsE7cGfQbp5AE/8xCIsS6Rg\n"+
        "DAOCAQUAAoIBAGBuLWV8iHFQN8o6YHMOFKv4u2Qw9OF0qlKaW4HBoeukO2iBe4m8\n"+
        "+69WHKNgaJo6jMLcS+FrATOw/7/2wbrex2xZQYfF0owxoQBINgwhfmLBcXELGZNu\n"+
        "SHd6voRCd/spafNDxSPbvIxqWlB7X3a56TVu/YcdQKGnjWQLXvLEtW4fapGAVqwm\n"+
        "hmguK/RBEYqIJJQ2wel+/w/jc2tEScRJ9oInxGqweppdbvx85AE4Cag/+8EjM3bx\n"+
        "bW6TTpHi6/HCr9nET7rEQlFo+FIHg9CngTW90JN4yIYPeo4QViCTr72hdNNjs44Q\n"+
        "09yqW9vogJBfPFysqwhMQms+xbAYsSEUMu8=\n"+
        "-----END PUBLIC KEY-----";
    
        dsa.parsePublicKey(pem);
    
        var pub_pem = dsa.exportPublicKey();
    
        assert.equal(pub_pem, pem, "DSA.parsePublicKey & exportPublicKey Test");
    ...
    //----------------------------------------------------------------------------------------------------
        ...
        var s = dsa.sign(k, { hashAlgo: hash_name });
    
        var v = dsa.verify(k, s, { hashAlgo: hash_name });
    
        assert.ok(v, "DSA.sign & verify with getPredefinedParams 1st Test");
    ...
    //----------------------------------------------------------------------------------------------------
        ...
        dsa.setParameters({
            p: "",
            q: "832124c3188298f8d6dd1ad84121c2ef33a5d52b",
    ...
    */
        ...
        var s = dsa.sign(k, { hashAlgo: 'sha-1' });
        var v = dsa.verify(k, s, { hashAlgo: 'sha-1' });
    
        assert.ok(v, "DSA.sign & verify with setParameters 2st Test");
    ...
    //----------------------------------------------------------------------------------------------------
        ...
        assert.ok(v, "DSA.sign & verify with parseParameters 3st Test");
