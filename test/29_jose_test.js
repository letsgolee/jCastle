...
function consoleJwt (jwt, l = 64)
{
    ...
    for (let i = 0; i < jwt_arr.length; i++) {
        const line = jwt_arr[i];
        res.push(jCastle.util.lineBreak(line, l));
    }
...
QUnit.module('JOSE');
QUnit.test('JWT Test', function(assert) {
...
// console.log('JWT Test 1\n');

var compact_jwt = jCastle.jose.jws.sign(payload, {
    algoName: 'RS256',
    key: rsa_priv_jwk,
    protectHeader: true
}, {
...
// console.log(v);
assert.ok(v, 'jwt test 1-1');

var jwt = jCastle.jose.jws.sign(payload, {
    algoName: 'RS256',
    key: rsa_priv_jwk,
    protectHeader: true
}, {
...
var jwk = `
    ...
    e8P_ijQ7p8Vdz0TTrxUeT3lm8d9shnr2lfJT8ImUjvAA2Xez2Mlp8cBE5awDzT
    0qI0n6uiP1aCN_2_jLAeQTlqRHtfa64QQSUmFAAjVKPbByi7xho0uTOcbH510a
    6GYmJUAfmWjwZ6oD4ifKo8DYM-X72Eaw`.replace(/[ \t\r\n]/g, '');
...
var jwt = `
    ...
    AE_R_YZCChjn4791jSQCrdPZCNYqHXCTZH0-JZGYNlaAjP2kqaluUIIUnC9qvb
    u9Plon7KRTzoNEuT4Va2cmL1eJAQy3mtPBu_u_sDDyYjnAMDxXPn7XrT0lw-kv
    AD890jl8e2puQens_IEKBpHABlsbEPX6sFY8OcGDqoRuBomu9xQ2`.replace(/[ \t\r\n]/g, '');
...
// console.log(v);
assert.ok(v, 'jwt test 4-1');

var general_jwt = jCastle.jose.jws.sign(payload, {
    algoName: 'HS256',
    key: mac_sig_jwk,
    protectHeader: true
}, {
...
// console.log(general_jwt);

var v = jCastle.jose.jws.verify(general_jwt, {
    key: mac_sig_jwk
});

// console.log(v);
assert.ok(v, 'jwt test 4-2');
...
UgeW91IG1pZ2h0IGJlIHN3ZXB0IG9mZiB0by4
.
s0h6KThzkfBBBkLspW1h84VsJZFTsPPqMDA7g1Md7p0`.replace(/[ \t\r\n]/g, '');
...
// console.log(v);
assert.ok(v, 'jwt test 6-1');

var flattened_jwt = jCastle.jose.jws.sign(payload, {
    algoName: 'HS256',
    key: mac_sig_jwk,
    //protectHeader: true
}, {
...
// console.log(flattened_jwt);

var v = jCastle.jose.jws.verify(flattened_jwt, {
    key: mac_sig_jwk
});

// console.log(v);
assert.ok(v, 'jwt test 6-2');
// console.log('\n--------------------------------------------------------\n');
...
// console.log(v);
assert.ok(v, 'jwt test 8-3');
// console.log('\n--------------------------------------------------------\n');


}); // end of JWT test

QUnit.test('JWE Test', function(assert) {
...
// The resulting JWE object using the general JWE JSON Serialization
// provided in the text is of wrong syntax, for the 'protected' property
// has JWE header values. It should be in the "header" of the 
// each recipient. For example the first recipient's algo is 'RSA1_5'
// and other recipient can have any different algo.
// https://datatracker.ietf.org/doc/html/rfc7516#appendix-A.4.7
//
// If you want to have the same result with the NIST example, 
// then put 'protectHeader' parameter and set it as true.

// console.log('JWE Test 1\n');

var compact_jwe = jCastle.jose.jwe.encrypt(plaintext, {
    algoName: 'RSA1_5',
    key: rsa_priv_jwk
}, {
...
assert.ok(v.toString() == plaintext, 'jwe test 1-1');
...
// console.log(v.toString() == plaintext);
assert.ok(v.toString() == plaintext, 'jwe test 1-2');

var general_jwe = jCastle.jose.jwe.encrypt(plaintext, {
    algoName: 'RSA1_5',
    key: rsa_priv_jwk
}, {
    algoName: 'A128CBC-HS256',
...
var cek_b64u = "mYMfsggkTAm0TbvtlFh2hyoXnbEzJQjMxmgLN3d8xXA";
var iv_b64u = "-nBoKLH0YkLZPSI9";
...
// console.log('JWE Test 2\n');

var compact_jwe = jCastle.jose.jwe.encrypt(plaintext, {
    algoName: 'RSA-OAEP',
    key: rsa_oaep_priv_jwk
}, {
    algoName: 'A256GCM',
    key: Buffer.from(cek_b64u, 'base64url'),
    iv: Buffer.from(iv_b64u, 'base64url')
...
// console.log(v.toString() == plaintext);
assert.ok(v.toString() == plaintext, 'jwe test 2-1');
...
maPpOslY2n525DxDfWaVFUfKQxMF56vn4B9QMpWAbnypNimbM8zVOw
.
UCGiqJxhBI3IFVdPalHHvA`.replace(/[ \t\r\n]/g, '');
...
// console.log(v.toString() == plaintext);
assert.ok(v.toString() == plaintext, 'jwe test 2-2');
...
var cek_b64u = "uwsjJXaBK407Qaf0_zpcpmr1Cs0CC50hIUEyGNEt3m0";
var iv_b64u = "VBiCzVHNoLiR3F4V82uoTQ";
...
var salt_b64u = "8Q1SzinasR3xchYz6ZZcHA";
var count = 8192;
...
AKYPqmXUeRdjFJwafkYEkiuDCV9vWGAi1DH2xTafhJwcmywIyzi4BqRpmdn_N-
zl5tuJYyuvKhjKv6ihbsV_k1hJGPGAxJ6wUpmwC4PTQ2izEm0TuSE8oMKdTw8V
3kobXZ77ulMwDs4p`.replace(/[ \t\r\n]/g, '');
var tag = `0HlwodAhOCILG5SQ2LQ9dg`;

var algo = 'aes-128';
var mode = 'cbc';
var cek = Buffer.from(cek_b64u, 'base64url');

var pt = jCastle.mcrypt.create().start({
    algoName: algo,
    mode: mode,
    isEncryption: false,
    key: cek.slice(cek.length / 2),
    iv: Buffer.from(iv_b64u, 'base64url'),
    padding: 'pkcs7'
}).update(Buffer.from(ct, 'base64url')).finalize();

console.log(pt.toString());
console.log(pt.toString('base64url'));
*/
/*
// plaintext result:
...

XX0
*/
...
// console.log('JWE Test 3\n');

var compact_jwe = jCastle.jose.jwe.encrypt(plaintext, {
    algoName: 'PBES2-HS512+A256KW',
    password: password,
    salt: Buffer.from(salt_b64u, 'base64url'),
    count: count,
    cty: "jwk-set+json",
}, {
    algoName: 'A128CBC-HS256',
...
// console.log(v.toString() == plaintext);
assert.ok(v.toString() == plaintext, 'jwe test 3-1');
...
3kobXZ77ulMwDs4p
.
0HlwodAhOCILG5SQ2LQ9dg`.replace(/[ \t\r\n]/g, '');
...
// console.log(v.toString() == plaintext);
assert.ok(v.toString() == plaintext, 'jwe test 3-2');
...
var v = jCastle.jose.jwe.decrypt(general_jwe, {
    algoName: 'PBES2-HS512+A256KW',
    password: password
});

// console.log(v.toString() == plaintext);
assert.ok(v.toString() == plaintext, 'jwe test 3-3');
// console.log('\n--------------------------------------------------------\n');
...
var cek_b64u = "Nou2ueKlP70ZXDbq9UrRwg";
var iv_b64u = "mH-G2zVqgztUtnW_";
...
var compact_jwe = jCastle.jose.jwe.encrypt(plaintext, {
    ...
    key: ec_ephemeral_priv_jwk,
    partyPublicKey: ec_priv_jwk
}, {
    algoName: 'A128GCM',
    key: Buffer.from(cek_b64u, 'base64url'),
    iv: Buffer.from(iv_b64u, 'base64url')
...
// console.log(v.toString() == plaintext);
assert.ok(v.toString() == plaintext, 'jwe test 4-1');
...
07WNhjzJEPc4jVntRJ6K53NgPQ5p99l3Z408OUqj4ioYezbS6vTPlQ
.
WuGzxmcreYjpHGJoa17EBg`.replace(/[ \t\r\n]/g, '');
...
// console.log(v.toString() == plaintext);
assert.ok(v.toString() == plaintext, 'jwe test 4-2');
...
// console.log(v.toString() == plaintext);
assert.ok(v.toString() == plaintext, 'jwe test 4-3');
// console.log('\n--------------------------------------------------------\n');
...
var iv_b64u = "yc9N8v5sYyv3iGQT926IUg";
...
var expected_cek_b64u = "hzHdlfQIAEehb8Hrd_mFRhKsKLEzPfshfXs9l6areCc";
...
// console.log('JWE Test 5\n');

// The issuer of jwe has the private key as ec_ephemeral_priv_jwk which has no kid.
// The other party has the private key as ec_priv_jwk which has the kid as "meriadoc.brandybuck@buckland.example".
// The issuer's private key has no kid so there should be no kid inside the jwe.
// But the NIST's JWE JOSE protected header has kid in it. see Figure 124.
// For the test, I will put 'kid' inside the key_algo_params.


var compact_jwe = jCastle.jose.jwe.encrypt(plaintext, {
    algoName: 'ECDH-ES',
    kid: "meriadoc.brandybuck@buckland.example", // for test. read the above comment.
    key: ec_ephemeral_priv_jwk,
    partyPublicKey: ec_priv_jwk
}, {
    algoName: 'A128CBC-HS256',
...
// console.log(v.toString() == plaintext);
assert.ok(v.toString() == plaintext, 'jwe test 5-1');
...
95_JGG2m9Csg
.
WCCkNa-x4BeB9hIDIfFuhg`.replace(/[ \t\r\n]/g, '');
...
// console.log(v.toString() == plaintext);
assert.ok(v.toString() == plaintext, 'jwe test 5-2');
...
// console.log(v.toString() == plaintext);
assert.ok(v.toString() == plaintext, 'jwe test 5-3');
//
// result: the NIST provided tag vector seems incorrect.
//         it should be 'NQQl_r5qvvKLJex2olPwNQ'.
//
...
// console.log(v.toString() == plaintext);
assert.ok(v.toString() == plaintext, 'jwe test 6-1');
...
ZI-wjsY0yu3cT4_aQ3i1o-tiE-F8Ios61EKgyIQ4CWao8PFMj8TTnp
.
vbb32Xvllea2OtmHAdccRQ`.replace(/[ \t\r\n]/g, '');
...
// console.log(v.toString() == plaintext);
assert.ok(v.toString() == plaintext, 'jwe test 6-2');
...
// console.log(v.toString() == plaintext);
assert.ok(v.toString() == plaintext, 'jwe test 6-3');
// console.log('\n--------------------------------------------------------\n');
...
var cek_b64u = 'UWxARpat23nL9ReIj4WG3D1ee9I4r-Mv5QLuFXdy_rE';
var iv_b64u = 'gz6NjyEFNm_vm8Gj6FwoFQ';
...
var wrapping_iv_b64u = 'KkYT0GX_2jHlfqN_';
...
// console.log('JWE Test 7\n');

var compact_jwe = jCastle.jose.jwe.encrypt(plaintext, {
    algoName: 'A256GCMKW',
    key: aes_key_jwk,
    iv: Buffer.from(wrapping_iv_b64u, 'base64url')
}, {
    algoName: 'A128CBC-HS256',
    key: Buffer.from(cek_b64u, 'base64url'),
    iv: Buffer.from(iv_b64u, 'base64url')
...
// console.log(v.toString() == plaintext);
assert.ok(v.toString() == plaintext, 'jwe test 7-1');
...
1B-gxpNk3xWU
.
DKW7jrb4WaRSNfbXVPlT5g`.replace(/[ \t\r\n]/g, '');
...
// console.log(v.toString() == plaintext);
assert.ok(v.toString() == plaintext, 'jwe test 7-2');
...
// console.log(v.toString() == plaintext);
assert.ok(v.toString() == plaintext, 'jwe test 7-3');
// console.log('\n--------------------------------------------------------\n');
...
var cek_b64u = "aY5_Ghmk9KxWPBLu_glx1w";
var iv_b64u = "Qx0pmsDa8KnJc9Jo";
...
// console.log(v.toString() == plaintext);
assert.ok(v.toString() == plaintext, 'jwe test 8-1');
...
a8Z7MOZ7UGxGIMvEmxrGCPeJa14slv2-gaqK0kEThkaSqdYw0FkQZF
.
ER7MWJZ1FBI_NKvn7Zb1Lw`.replace(/[ \t\r\n]/g, '');
...
// console.log(v.toString() == plaintext);
assert.ok(v.toString() == plaintext, 'jwe test 8-2');
...
// console.log(v.toString() == plaintext);
assert.ok(v.toString() == plaintext, 'jwe test 8-3');
// console.log('\n--------------------------------------------------------\n');
...
var compressed_pt_b64u = `
bY_BDcIwDEVX-QNU3QEOrIA4pqlDokYxchxVvbEDGzIJbioOSJwc-f___HPjBu
8KVFpVtAplVE1-wZo0YjNZo3C7R5v72pV5f5X382VWjYQpqZKAyjziZOr2B7kQ
PSy6oZIXUnDYbVKN4jNXi2u0yB7t1qSHTjmMODf9QgvrDzfTIQXnyQRuUya4zI
WG3vTOdir0v7BRHFYWq3k1k1A_gSDJqtcBF-GZxw8`.replace(/[ \t\r\n]/g, '');

var cek_b64u = "hC-MpLZSuwWv8sexS6ydfw";
var iv_b64u = "p9pUq6XHY0jfEZIl";
...
// console.log('JWE Test 9\n');

var compressed_pt = Buffer.from(compressed_pt_b64u, 'base64url');

var compact_jwe = jCastle.jose.jwe.encrypt(compressed_pt, {
    algoName: 'A128KW',
    key: aes_key_jwk,
    zip: "DEF"
}, {
...
// console.log(v.toString() == compressed_pt);
assert.ok(v.toString() == compressed_pt, 'jwe test 9-1');
...
hpYA7qi3AyijnCJ7BP9rr3U8kxExCpG3mK420TjOw
.
VILuUwuIxaLVmh5X-T7kmA`.replace(/[ \t\r\n]/g, '');
...
// console.log(v.toString() == compressed_pt);
assert.ok(v.toString() == compressed_pt, 'jwe test 9-2');

var general_jwe = jCastle.jose.jwe.encrypt(compressed_pt, {
    algoName: 'A128KW',
    key: aes_key_jwk,
    zip: "DEF"
}, {
...
// console.log(v.toString() == compressed_pt);
assert.ok(v.toString() == compressed_pt, 'jwe test 9-3');
// console.log('\n--------------------------------------------------------\n');
...
var aad_b64u = Buffer.from(JSON.stringify([
    "vcard",
    [
      [ "version", {}, "text", "4.0" ],
      [ "fn", {}, "text", "Meriadoc Brandybuck" ],
...
var cek_b64u = "75m1ALsYv10pZTKPWrsqdg";
var iv_b64u = "veCx9ece2orS7c_N";
var expected_aad_b64u = `
WyJ2Y2FyZCIsW1sidmVyc2lvbiIse30sInRleHQiLCI0LjAiXSxbImZuIix7fS
widGV4dCIsIk1lcmlhZG9jIEJyYW5keWJ1Y2siXSxbIm4iLHt9LCJ0ZXh0Iixb
IkJyYW5keWJ1Y2siLCJNZXJpYWRvYyIsIk1yLiIsIiJdXSxbImJkYXkiLHt9LC
J0ZXh0IiwiVEEgMjk4MiJdLFsiZ2VuZGVyIix7fSwidGV4dCIsIk0iXV1d`.replace(/[ \t\r\n]/g, '');

//console.log(aad_b64u == expected_aad_b64u);
...
// console.log(v.toString() == plaintext);
assert.ok(v.toString() == plaintext, 'jwe test 10-1');
// console.log('\n--------------------------------------------------------\n');
...
var cek_b64u = "WDgEptBmQs9ouUvArz6x6g";
var iv_b64u = "WgEJsDS9bkoXQ3nR";
...
// console.log(general_jwe);
// console.log(general_jwe.recipients[0].header);
...
// console.log(v.toString() == plaintext);
assert.ok(v.toString() == plaintext, 'jwe test 11-1');
// console.log('\n--------------------------------------------------------\n');
...
// jCastle.jose does not support this JWE structure.
// It's in conflict with the JWE structure that RFC 7516 explains.
...
var cek_b64u = "zXayeJ4gvm8NJr3IUInyokTUO-LbQNKEhe_zWlYbdpQ";
var iv_b64u = "VgEIHY20EnzUtZFl2RpB1g";
...
// NIST's EC key explanation problem:
//
// According to RFC 7518(JWA) epk should be contain the originator's ephemeral
// public key information.
// See section 4.6.1.1.  "epk" (Ephemeral Public Key) Header Parameter. It says:
//     The "epk" (ephemeral public key) value created by the originator for
//     the use in key agreement algorithms.
// Then kid in the epk should belong to the originator, but the originator's 
// ephemeral private key has no kid.
// In Figure 209, there is kid and this kid comes from the other party's public key!
// So for the test, I will put kid to the test code manually.
...
5.13.5.  Encrypting the Key to the Third Recipient
   ...
   AvpeoPZ9Ncn9mkBn
     ...
     Figure 211: Recipient #2 Initialization Vector for Key Wrapping,
                             base64url-encoded
*/
var wrapping_iv_b64u = "AvpeoPZ9Ncn9mkBn";
/*
...
// console.log('JWE Test 13\n');

var general_jwe = jCastle.jose.jwe.encrypt(plaintext, [ // keywrapping algorithms
    {
        algoName: 'RSA1_5',
        key: rsa_priv_jwk
    },{
        algoName: 'ECDH-ES+A256KW',
        key: ec_ephemeral_priv_jwk,
        kid: "peregrin.took@tuckborough.example", // for the test
        partyPublicKey: ec_priv_jwk
    },{
        algoName: 'A256GCMKW',
        key: aes_key_jwk,
...
// check each header
// console.log(general_jwe.recipients[0].header);
// console.log(general_jwe.recipients[1].header);
// console.log(general_jwe.recipients[2].header);
...
// console.log(v.toString() == plaintext);
assert.ok(v.toString() == plaintext, 'jwe test 13-1');
...
// console.log(v.toString() == plaintext);
assert.ok(v.toString() == plaintext, 'jwe test 13-2');
...
// console.log(v.toString() == plaintext);
assert.ok(v.toString() == plaintext, 'jwe test 13-3');
// console.log('\n--------------------------------------------------------\n');
...
var payload_b64u = `
eyJpc3MiOiJob2JiaXRvbi5leGFtcGxlIiwiZXhwIjoxMzAwODE5MzgwLCJodH
RwOi8vZXhhbXBsZS5jb20vaXNfcm9vdCI6dHJ1ZX0`.replace(/[ \t\r\n]/g, '');

// console.log(payload == Buffer.from(payload_b64u, 'base64url').toString());
...
// console.log('JWS/JWE Test 1\n');

var signature_jwt = jCastle.jose.jws.sign(payload, {
    algoName: 'PS256',
    typ: "JWT",
    key: rsa_priv_jwk,
    protectHeader: true
...
var cek_b64u = "0RHSNYwN-6-2QBGsYTZLSQ";
var iv_b64u = "GbX1i9kXz0sxXPmA";
...
var encrypted_jwe = jCastle.jose.jwe.encrypt(signature_jwt, {
    algoName: 'RSA-OAEP',
    key: rsa_priv_jwk,
    typ: "JWT"
}, {
...
// consoleJwt(encrypted_jwe);

var v = jCastle.jose.jwe.decrypt(encrypted_jwe, {
    algoName: 'RSA-OAEP',
    key: rsa_priv_jwk
});

// console.log(v.toString() == signature_jwt);
assert.ok(v.toString() == signature_jwt, 'jwt/jwe test 1-1');

var v = jCastle.jose.jws.verify(signature_jwt, {
    key: rsa_priv_jwk
});

// console.log(v);
assert.ok(v, 'jwt/jwe test 1-2');
...
// console.log(v.toString() == signature_jwt);
assert.ok(v.toString() == signature_jwt, 'jwt/jwe test 1-3');
// console.log('\n--------------------------------------------------------\n');
